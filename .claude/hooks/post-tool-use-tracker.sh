...
# Customized for A4C-AppSuite monorepo structure
...
# Function to detect repo from file path - CUSTOMIZED FOR A4C-APPSUITE
detect_repo() {
    local file="$1"
    ...
    # A4C-AppSuite specific structure
    case "$repo" in
        ...
        infrastructure)
            echo "infrastructure"
            ;;
        # Dev directory (development docs)
        dev)
            echo "dev"
            ;;
        *)
...
get_build_command() {
    ...
    if [[ "$repo" == "frontend" ]] && [[ -f "$repo_path/package.json" ]]; then
    ...
    # Temporal: npm run build
    if [[ "$repo" == "temporal" ]] && [[ -f "$repo_path/package.json" ]]; then
        if grep -q '"build"' "$repo_path/package.json" 2>/dev/null; then
            echo "cd $repo_path && npm run build"
            return
        fi
...
get_tsc_command() {
    ...
    if [[ "$repo" == "frontend" ]] && [[ -f "$repo_path/tsconfig.json" ]]; then
        # Check for Vite-specific tsconfig
    ...
    # Temporal: TypeScript check
    if [[ "$repo" == "temporal" ]] && [[ -f "$repo_path/tsconfig.json" ]]; then
        echo "cd $repo_path && npx tsc --noEmit"
        return
    fi
