/**
 * Organization Pages UAT
 *
 * Tests for OrganizationListPage (/organizations) and OrganizationsManagePage (/organizations/manage).
 * Updated for the route-split refactor (Phase 3) and card-based create form (Phase 4).
 *
 * Run with:
...
/** Navigate to the manage page (/organizations/manage) and wait for it to render */
...
/** Navigate to the list page (/organizations) and wait for it to render */
async function navigateToListPage(page: Page, params?: string) {
  ...
  await page.waitForSelector('[data-testid="org-list-page"]', { timeout: 15000 });
}

/** Wait for the organization card grid to be populated on the list page */
async function waitForCardsLoaded(page: Page) {
  ...
  try {
    ...
    // Loading indicator may not appear if data loads instantly
  }
  await page.waitForSelector('[data-testid="organization-list"]', { timeout: 10000 });
}

/** Wait for org details to finish loading on the manage page */
...
/** Navigate to manage page with orgId param and wait for details to load */
async function selectOrg(page: Page, orgId: string) {
  await navigateToManagePage(page, `orgId=${orgId}`);
...
test.describe('TS-01: Navigation & Page Load', () => {
  ...
  test('TC-01-03: provider_admin is redirected from list page to manage page', async ({ page }) => {
    await navigateToManagePage(page);
  ...
  test('TC-01-05: Back button navigates to /organizations (list page)', async ({ page }) => {
    await navigateToManagePage(page);
...
// ============================================================================
// TS-02: Organization List [List Page] (9 cases)
// ============================================================================
test.describe('TS-02: Organization List', () => {
  ...
  test('TC-02-01: List loads with 9 org cards (platform_owner excluded)', async ({ page }) => {
    const cards = page.locator(
      '[data-testid="organization-list"] [data-testid^="org-card-"][data-slot="card"]'
    );
    await expect(cards).toHaveCount(9);
  });

  test('TC-02-02: Each card shows display_name, type badge, and status badge', async ({ page }) => {
    const card = page.locator('[data-testid="org-card-provider-abc-healthcare-id"]');
    await expect(card.locator('[data-testid="org-card-name"]')).toHaveText('ABC Healthcare');
    await expect(card.locator('[data-testid="org-card-type-badge"]')).toHaveText('Provider');
    await expect(card.locator('[data-testid="org-card-status-badge"]')).toContainText('Active');
  });

  test('TC-02-03: Active filter shows 8 cards (Summit Health excluded)', async ({ page }) => {
    await page.click('[data-testid="org-list-filter-active"]');
    const cards = page.locator(
      '[data-testid="organization-list"] [data-testid^="org-card-"][data-slot="card"]'
    );
    await expect(cards).toHaveCount(8);
    await expect(
      page.locator('[data-testid="org-card-provider-summit-health-id"]')
    ).not.toBeVisible();
  });

  test('TC-02-04: Inactive filter shows 1 card (Summit Health only)', async ({ page }) => {
    await page.click('[data-testid="org-list-filter-inactive"]');
    const cards = page.locator(
      '[data-testid="organization-list"] [data-testid^="org-card-"][data-slot="card"]'
    );
    await expect(cards).toHaveCount(1);
    await expect(page.locator('[data-testid="org-card-provider-summit-health-id"]')).toBeVisible();
  });

  test('TC-02-05: All filter shows 9 cards after switching from inactive filter', async ({
    page,
  }) => {
    await page.click('[data-testid="org-list-filter-inactive"]');
    await page.click('[data-testid="org-list-filter-all"]');
    const cards = page.locator(
      '[data-testid="organization-list"] [data-testid^="org-card-"][data-slot="card"]'
    );
    await expect(cards).toHaveCount(9);
  ...
  test('TC-02-06: Search "ABC" filters to 1 result: ABC Healthcare', async ({ page }) => {
    ...
    await expect(cards).toHaveCount(1);
    await expect(page.locator('[data-testid="org-card-provider-abc-healthcare-id"]')).toBeVisible();
  });
  ...
  test('TC-02-07: Search "xyz" (case-insensitive) matches XYZ Medical', async ({ page }) => {
    ...
    await expect(cards).toHaveCount(1);
    await expect(page.locator('[data-testid="org-card-provider-xyz-medical-id"]')).toBeVisible();
  });

  test('TC-02-08: Search "nonexistent" shows empty state', async ({ page }) => {
  ...
  test('TC-02-09: Card click navigates to manage page with orgId', async ({ page }) => {
    await page.click('[data-testid="org-card-provider-abc-healthcare-id"]');
    await page.waitForURL(/\/organizations\/manage\?orgId=provider-abc-healthcare-id/, {
      timeout: 5000,
    });
    await page.waitForSelector('[data-testid="org-manage-page"]', { timeout: 15000 });
  });
...
test.describe('TS-03: Form Field Editability', () => {
  test('TC-03-01: super_admin selects active org -- all fields enabled', async ({ page }) => {
    await selectOrg(page, 'provider-abc-healthcare-id');
  ...
  test('TC-03-04: Inactive org (Summit Health) shows inactive banner and all fields disabled', async ({
    ...
    await selectOrg(page, 'provider-summit-health-id');
...
test.describe('TS-06: Unsaved Changes Guard', () => {
  test('TC-06-01: Clicking Create with unsaved changes triggers discard dialog', async ({
    page,
  }) => {
    await selectOrg(page, 'provider-abc-healthcare-id');
    await page.fill('#org-display-name', 'Modified');
    await page.click('[data-testid="org-manage-create-btn"]');
  ...
  test('TC-06-03: "Discard Changes" enters create mode', async ({ page }) => {
    await selectOrg(page, 'provider-abc-healthcare-id');
    await page.fill('#org-display-name', 'Modified');
    await page.click('[data-testid="org-manage-create-btn"]');
    await page.locator('[data-testid="confirm-dialog"]').waitFor({ timeout: 5000 });
    ...
    await expect(page.locator('[data-testid="confirm-dialog"]')).not.toBeVisible();
    await expect(page.locator('[data-testid="org-create-form"]')).toBeVisible({ timeout: 5000 });
    await expect(page.locator('[data-testid="org-details-card"]')).not.toBeVisible();
  });

  test('TC-06-04: Clicking Create without unsaved changes enters create mode immediately', async ({
    page,
  }) => {
    await selectOrg(page, 'provider-abc-healthcare-id');
...
test.describe('TS-16: Keyboard Navigation & Accessibility', () => {
  test('TC-16-01: Tab from back button reaches Create button on manage page', async ({ page }) => {
    await navigateToManagePage(page);
    const backBtn = page.locator('[data-testid="org-manage-back-btn"]');
    await backBtn.focus();
    await expect(backBtn).toBeFocused();
    // Tab to Create button in page header
    await page.keyboard.press('Tab');
    await expect(page.locator('[data-testid="org-manage-create-btn"]')).toBeFocused();
  });

  test('TC-16-02: List page filter group has role=group and aria-label', async ({ page }) => {
    await navigateToListPage(page);
    await waitForCardsLoaded(page);
  ...
  test('TC-16-03: List page search has role=search landmark', async ({ page }) => {
    await navigateToListPage(page);
    await waitForCardsLoaded(page);
...
test.describe('TS-17: URL Parameter Handling', () => {
  test('TC-17-01: ?status=inactive on list page pre-selects Inactive filter', async ({ page }) => {
    await navigateToListPage(page, 'status=inactive');
    await waitForCardsLoaded(page);
    await expect(page.locator('[data-testid="org-list-filter-inactive"]')).toHaveAttribute(
      'aria-pressed',
      'true'
    );
    const cards = page.locator(
  ...
  test('TC-17-02: ?orgId=provider-abc-healthcare-id on manage page auto-selects ABC Healthcare', async ({
    page,
  }) => {
...
/** Click the Create button on the manage page and wait for the create form */
 ...
 * Kept for backward compat with tests that may need it for edge cases.
 */
...
test.describe('TS-18: Create Button Visibility & Entry', () => {
  test('TC-18-01: Create button visible for super_admin on manage page', async ({ page }) => {
    await navigateToManagePage(page);
    await expect(page.locator('[data-testid="org-manage-create-btn"]')).toBeVisible();
  });

  test('TC-18-02: Create button visible for super_admin on list page', async ({ page }) => {
    await navigateToListPage(page);
    await expect(page.locator('[data-testid="org-list-create-btn"]')).toBeVisible();
  });

  test('TC-18-03: Clicking Create on manage page shows the create form', async ({ page }) => {
    await navigateToManagePage(page);
    await enterCreateMode(page);
    await expect(page.locator('[data-testid="org-create-form"]')).toBeVisible();
  });

  test('TC-18-04: Create form replaces empty state', async ({ page }) => {
    await navigateToManagePage(page);
    await expect(page.locator('[data-testid="org-form-empty-state"]')).toBeVisible();
    await enterCreateMode(page);
    await expect(page.locator('[data-testid="org-form-empty-state"]')).not.toBeVisible();
...
// ============================================================================
// TS-19: Create Form Structure & Sections (4 cases)
// ============================================================================
test.describe('TS-19: Create Form Structure & Sections', () => {
  ...
  test('TC-19-01: Organization Details card visible by default', async ({ page }) => {
...
// ============================================================================
// TS-24: Create Mode Entry via URL & Back Navigation (3 cases)
// ============================================================================
test.describe('TS-24: Create Mode Entry via URL & Back Navigation', () => {
  test('TC-24-01: ?mode=create on manage page enters create mode directly', async ({ page }) => {
    await navigateToManagePage(page, 'mode=create');
    await expect(page.locator('[data-testid="org-create-form"]')).toBeVisible({ timeout: 5000 });
    await expect(page.locator('[data-testid="org-form-empty-state"]')).not.toBeVisible();
  });

  test('TC-24-02: ?mode=create takes precedence over ?orgId= (S7 collision guard)', async ({
    page,
  }) => {
    await navigateToManagePage(page, 'mode=create&orgId=provider-abc-healthcare-id');
    await expect(page.locator('[data-testid="org-create-form"]')).toBeVisible({ timeout: 5000 });
    await expect(page.locator('[data-testid="org-details-card"]')).not.toBeVisible();
  });

  test('TC-24-03: List page Create button navigates to manage page with ?mode=create', async ({
    page,
  }) => {
    await navigateToListPage(page);
    await waitForCardsLoaded(page);
    await page.click('[data-testid="org-list-create-btn"]');
    await page.waitForURL(/\/organizations\/manage\?mode=create/, { timeout: 5000 });
    await page.waitForSelector('[data-testid="org-create-form"]', { timeout: 10000 });
  });
});

// ============================================================================
// TS-25: Organization Card Details [List Page] (5 cases)
// ============================================================================
test.describe('TS-25: Organization Card Details', () => {
  test.beforeEach(async ({ page }) => {
    await navigateToListPage(page);
    await waitForCardsLoaded(page);
  });

  test('TC-25-01: Card shows provider admin name and email', async ({ page }) => {
    const card = page.locator('[data-testid="org-card-provider-abc-healthcare-id"]');
    await expect(card.locator('[data-testid="org-card-admin-name"]')).toContainText(
      'Dr. Sarah Chen'
    );
    await expect(card.locator('[data-testid="org-card-admin-email"]')).toContainText(
      'sarah.chen@abchealthcare.com'
    );
  });

  test('TC-25-02: Card shows provider admin phone when present', async ({ page }) => {
    const card = page.locator('[data-testid="org-card-provider-abc-healthcare-id"]');
    await expect(card.locator('[data-testid="org-card-admin-phone"]')).toContainText(
      '(555) 234-5678'
    );
  });

  test('TC-25-03: Card shows "No admin assigned" when no provider admin', async ({ page }) => {
    const card = page.locator('[data-testid="org-card-provider-summit-health-id"]');
    await expect(card.locator('[data-testid="org-card-no-admin"]')).toBeVisible();
    await expect(card.locator('[data-testid="org-card-no-admin"]')).toContainText(
      'No admin assigned'
    );
  });

  test('TC-25-04: Inactive org card shows Inactive badge', async ({ page }) => {
    const card = page.locator('[data-testid="org-card-provider-summit-health-id"]');
    await expect(card.locator('[data-testid="org-card-status-badge"]')).toContainText('Inactive');
  });

  test('TC-25-05: Partner org card shows Partner type badge', async ({ page }) => {
    const card = page.locator('[data-testid="org-card-var-partner-techsolutions-id"]');
    await expect(card.locator('[data-testid="org-card-type-badge"]')).toHaveText('Partner');
  });
});

// ============================================================================
// TS-26: List Page Search by Admin Fields (3 cases)
// ============================================================================
test.describe('TS-26: List Page Search by Admin Fields', () => {
  test.beforeEach(async ({ page }) => {
    await navigateToListPage(page);
    await waitForCardsLoaded(page);
  });

  test('TC-26-01: Search by provider admin name "Sarah" finds ABC Healthcare', async ({ page }) => {
    await page.fill('[data-testid="org-list-search-input"]', 'Sarah');
    const cards = page.locator(
      '[data-testid="organization-list"] [data-testid^="org-card-"][data-slot="card"]'
    );
    await expect(cards).toHaveCount(1);
  ...
  test('TC-26-02: Search by admin email "mrodriguez" finds XYZ Medical', async ({ page }) => {
    await page.fill('[data-testid="org-list-search-input"]', 'mrodriguez');
    const cards = page.locator(
      '[data-testid="organization-list"] [data-testid^="org-card-"][data-slot="card"]'
    );
    await expect(cards).toHaveCount(1);
  ...
  test('TC-26-03: Search by subdomain "healthit" finds HealthIT card', async ({ page }) => {
    await page.fill('[data-testid="org-list-search-input"]', 'healthit');
    const cards = page.locator(
      '[data-testid="organization-list"] [data-testid^="org-card-"][data-slot="card"]'
    );
    await expect(cards).toHaveCount(1);
    await expect(page.locator('[data-testid="org-card-var-partner-healthit-id"]')).toBeVisible();
  });
});

// ============================================================================
// TS-27: List Page Filter Tabs with aria-pressed (3 cases)
// ============================================================================
test.describe('TS-27: List Page Filter Tabs', () => {
  test.beforeEach(async ({ page }) => {
    await navigateToListPage(page);
    await waitForCardsLoaded(page);
  });

  test('TC-27-01: All filter has aria-pressed=true by default', async ({ page }) => {
    await expect(page.locator('[data-testid="org-list-filter-all"]')).toHaveAttribute(
      'aria-pressed',
      'true'
    );
    await expect(page.locator('[data-testid="org-list-filter-active"]')).toHaveAttribute(
      'aria-pressed',
      'false'
    );
    await expect(page.locator('[data-testid="org-list-filter-inactive"]')).toHaveAttribute(
  ...
  test('TC-27-02: Clicking Active filter sets aria-pressed=true and deselects All', async ({
    page,
  }) => {
    await page.click('[data-testid="org-list-filter-active"]');
    await expect(page.locator('[data-testid="org-list-filter-active"]')).toHaveAttribute(
  ...
  test('TC-27-03: Filter tabs display correct counts', async ({ page }) => {
    await expect(page.locator('[data-testid="org-list-filter-all"]')).toContainText('9');
    await expect(page.locator('[data-testid="org-list-filter-active"]')).toContainText('8');
    await expect(page.locator('[data-testid="org-list-filter-inactive"]')).toContainText('1');
  });
});
