...
export const FocusTrappedCheckboxGroup = observer(
  ({
    ...
    const getCheckboxItemClasses = (index: number) =>
      cn(
    ...
    return (
      <div
        ...
        aria-errormessage={
          hasError && (errorMessage || errorMessageId) ? errorMessageId || `${id}-error` : undefined
        }
      ...
      >
        ...
        {/* Help Text */}
        {helpText && (
          <p id={helpTextId || `${id}-help`} className="text-sm text-gray-600 mt-2">
            {helpText}
          </p>
        )}
              ...
              {items.map((item, index) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <span className={cn('text-sm select-none', item.disabled && 'text-gray-400')}>
                      {item.label}
                    </span>
                    {item.description && (
                      <p id={`${id}-${item.id}-desc`} className="text-xs text-gray-500 mt-1">
                        {item.description}
                      </p>
                    )}
                  </div>
