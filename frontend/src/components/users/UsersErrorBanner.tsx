 ...
 *
 * The container is programmatically focusable (`tabIndex={-1}`) and accepts a
 * forwarded ref so the page can move focus here on a form-blocking submit
 * failure (via `useEffect`, never `setTimeout`) — matching the plain
 * `CommandFeedbackBanner`. See `documentation/frontend/patterns/command-feedback.md`.
 */
...
export const UsersErrorBanner = observer(
  forwardRef<HTMLDivElement, UsersErrorBannerProps>(function UsersErrorBanner(
    { error, operationError, lastRoleViolations, lastRolePartialFailure, onDismiss },
    ref
  ) {
    ...
    return (
      <div
        ref={ref}
        tabIndex={-1}
        className="mb-6 p-4 rounded-lg border border-red-300 bg-red-50 outline-none"
        role="alert"
            ...
            {lastRoleViolations && lastRoleViolations.length > 0 ? (
              ...
              <div data-testid="role-modification-partial-warning">
                ...
                <p className="text-red-700 text-sm mt-1">
                  ...
                  {lastRolePartialFailure.failureIndex}. Re-running with the same selections is
                  safe; events already emitted ({lastRolePartialFailure.addedRoleEventIds.length}{' '}
                  added, {lastRolePartialFailure.removedRoleEventIds.length} removed) will be
                  no-ops.
                </p>
