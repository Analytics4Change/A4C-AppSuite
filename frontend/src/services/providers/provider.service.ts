...
class ProviderService {
   ...
   * Note: Organization ID is generated by the database
   */
  async createProvider(request: CreateProviderRequest, organizationId: string): Promise<Provider> {
    try {
      log.info('Creating provider', { name: request.name, organizationId });

      const client = await supabaseService.getClient();

      const providerData = {
        id: organizationId, // Use provided organization ID
        name: request.name,
        type: request.type,
