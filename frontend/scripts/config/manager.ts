...
/**
 * Recursive partial — mirrors `mergeConfigs`' deep-merge behaviour so callers can
 * override a single nested field (e.g. `{ logging: { level: 'debug' } }`) without
 * supplying the whole section. Arrays are replaced wholesale (not deep-merged).
 */
type DeepPartial<T> = {
...
export class ConfigManager {
   ...
   */
  override(overrides: DeepPartial<ScriptConfig>): void {
   ...
   */
  private mergeConfigs(base: ScriptConfig, overrides: DeepPartial<ScriptConfig>): ScriptConfig {
    const result = { ...base };
