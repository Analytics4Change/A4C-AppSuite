 ...
 *
 * Orchestrates organization provisioning with DNS and user invitations.
 *
 * Flow:
 ...
 *
 * Idempotency:
 * - Workflow ID should be unique per organization (e.g., org-bootstrap-{subdomain})
...
export async function organizationBootstrapWorkflow(
  ...
  try {
    ...
    log.info('Organization created', { orgId: state.orgId });

    // ========================================
    // Step 2: Configure DNS (with retries)
    // ========================================
    ...
    // Compensation: Deactivate organization if created
