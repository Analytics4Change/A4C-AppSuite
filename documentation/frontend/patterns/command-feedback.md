...
- 🔄 **Phase 3 (in progress)** — progressive-enhancement rollout to the other command-feedback pages, batched by area. Existing banners already surface every failure, so partial rollout is never broken — only less uniform.
  ...
  - Remaining targets (seeded follow-ups): the **Organizations *create* flow** (`OrganizationCreateForm` — separate component/VM); and the shared **`sanitizeCommandError` SQLSTATE heuristic** false-positive (a field name like `A1234` trips `\b[A-Z]\d{4}\b`).
