...
- 🔄 **Phase 3 (in progress)** — progressive-enhancement rollout to the other command-feedback pages, batched by area. Existing banners already surface every failure, so partial rollout is never broken — only less uniform.
  ...
  - **Organizations *create* flow** (`OrganizationCreateForm`): migrated — the raw `submissionError` banner is now the shared `<CommandFeedbackBanner kind="error">` (sanitized) + `aria-hidden` echo + focus-to-banner via `useCommandFeedbackFocus`; success navigates to the bootstrap status page (no same-page success banner, like `ClientIntakePage`). The correlation id is **server-generated** (by the Edge Function, returned via the `x-correlation-id` header) and, when present, is surfaced to the user *inside* the error message as a support ref (`(Ref: <id>)`, appended by `TemporalWorkflowClient`) — a `UUID` doesn't trip the sanitizer's SQLSTATE heuristic, so it passes through. It is **not** passed as a structured `correlationId` to the presentation logger; wiring that off the VM seam is the read-path correlation-id follow-up.
