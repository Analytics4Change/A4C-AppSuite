...
- 🔄 **Phase 3 (in progress)** — progressive-enhancement rollout to the other command-feedback pages, batched by area. Existing banners already surface every failure, so partial rollout is never broken — only less uniform.
  ...
  - Remaining targets: **Tier-2** surfaces (settings client-fields, assignments caseload, client intake, admin orphaned-deletions, auth reset-password toast→banner) and the **Organizations *create* flow** (`OrganizationCreateForm` — a separate component/VM from the manage page).
