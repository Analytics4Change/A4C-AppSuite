...
| correlation_id | uuid | YES | - | Business-scoped correlation_id for the user's identity/membership lifecycle (added 2026-06-23). Anchored at `user.created`, reused by identity/membership emitters. See [event-metadata-schema.md](../../../../workflows/reference/event-metadata-schema.md#events-using-stored-correlation_id). |
