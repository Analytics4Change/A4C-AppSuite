...
<!-- DO NOT HAND-EDIT — content between START/END is regenerated by
     frontend/scripts/gen-rpc-reachability-matrix.cjs (Phase 1 step 12).
     The legacy hand-curated 5-column table below (with `r/w` and `Notes`
     columns) is preserved as the pre-codegen state. First codegen run
     replaces it with a simpler 2-column (Bucket, Count) derived from
     pg_description @a4c-bucket tags. The rich `r/w` and `Notes` columns
     are NOT @a4c-tag-derivable and may be moved to a sibling table or a
     follow-up enhancement that adds @a4c-rw and @a4c-bucket-note tags. -->
...
| D | 32 | 4 | **36** | Phase 4 RLS audit target. Stage R reconciliation: +2 net from missing-72 (`get_organization_details`, `list_schedule_templates`); -2 stale (`get_schedule_by_id`, `list_user_schedules` — replaced by schedule_template + assignment model). Stage R-6 F1 fold-in 2026-05-30: `check_field_definitions_exist` + `deactivate_all_field_definitions` moved D→E `[service-role-only]` (their only enforcement is `GRANT EXECUTE TO service_role` — no `authenticated` grant; structurally identical to `safety_net_deactivate_organization`). |
| D-variant | 1 | 0 | **1** | `get_user_addresses_for_org` — `has_platform_privilege()` + explicit RAISE on insufficient perms + RLS on `user_addresses`. The combination of platform-privilege check AND load-bearing RLS makes it sui generis: the platform-privilege side acts as an "admin override" while non-admin callers fall through to RLS enforcement on `user_addresses`. Stage R-6 F2 fold-in 2026-05-30: the 4 Stage R additions (`deactivate_organization`, `delete_organization`, `reactivate_organization`, `retry_deletion_workflow`) moved D-variant→E `[admin-only]` (their `has_platform_privilege()` gate is the ONLY enforcement — no load-bearing RLS contribution; structurally identical to existing `retry_failed_event` / `dismiss_failed_event` E-classification). |
| E | 24 | 19 | **43** | Stage R reconciliation: +6 from missing-72 (`get_failed_events_with_detail` `[admin-only]`, `get_orphaned_deletions` `[admin-only]`, `list_field_definition_templates` reference data, `list_system_field_categories` reference data, `deactivate_user` [unscoped `has_permission` + manual tenancy guard; matches `delete_user` precedent], `safety_net_deactivate_organization` `[service-role-only]`). Stage R-6 F1+F2 fold-in 2026-05-30: +2 reads from F1 (`check_field_definitions_exist`, `deactivate_all_field_definitions` `[service-role-only]`) and +4 writes from F2 (org-lifecycle quartet — all `[admin-only]`). |
| E-variant | 1 | 0 | **1** | `list_user_organizations` — sui generis (mixed self-context + org-admin predicate) |
| **Total** | **78** | **92** | **170** | Matches live pg_proc distinct count exactly (170 distinct names; F1+F2 are intra-table reshuffles — total unchanged). |
...
<!-- DO NOT HAND-EDIT — content between START/END is regenerated by
     frontend/scripts/gen-rpc-reachability-matrix.cjs (Phase 1 step 12).
     First codegen run replaces this rich 5-column table with a 5-column
     table derived from @a4c-* tags (name, bucket, callable, phase-target,
     reason). The pre-codegen rich `guard / source-of-tenancy` column has
     value (cites exact migration line numbers) but is NOT @a4c-derivable.
     Either re-encode it as @a4c-source-citation tag in a follow-up, or
     accept the simpler schema. -->
...
| `check_field_definitions_exist` | E | R | `GRANT EXECUTE ... TO service_role` only; no `authenticated` grant (`20260330191322_fix_seed_field_definitions_idempotency_check.sql:30`) | Check whether org has field definitions (Temporal seed-activity caller) [service-role-only] |
...
| `deactivate_all_field_definitions` | E | R | `GRANT EXECUTE ... TO service_role` only; no `authenticated` grant (`20260330170946_fix_seed_field_definitions_schema_access.sql:121`) | Deactivate all field definitions for org (Temporal compensation activity caller) [service-role-only] |
...
| `deactivate_organization` | E | W | `has_platform_privilege()` early-return gate; RLS not load-bearing (`20260226002002_organization_manage_page_phase1.sql:390`) | Deactivate organization (admin) [admin-only] |
...
| `delete_organization` | E | W | `has_platform_privilege()` early-return gate; RLS not load-bearing (`20260226002002_organization_manage_page_phase1.sql:534`) | Hard-delete organization (admin) [admin-only] |
...
| `get_organization_details` | D | R | none (SECURITY DEFINER bypasses caller-RLS); RLS on `organizations_projection` is informational only; `p_org_id` parameter is taken at face value (`20260226002002_organization_manage_page_phase1.sql:214`) | Fetch org details with extended metadata — see Phase 4 sub-audit § definer-bypasses-RLS |
...
| `list_schedule_templates` | D | R | none (SECURITY DEFINER bypasses caller-RLS); RLS on `schedule_templates_projection` is informational only; `COALESCE(p_org_id, get_current_org_id())` is taken at face value with no permission check (`20260218001058_denormalize_schedule_assigned_user_count.sql:164`) | List schedule templates with assigned-user counts — see Phase 4 sub-audit § definer-bypasses-RLS |
...
| `reactivate_organization` | E | W | `has_platform_privilege()` early-return gate; RLS not load-bearing (`20260226002002_organization_manage_page_phase1.sql:466`) | Reactivate organization (admin) [admin-only] |
...
| `retry_deletion_workflow` | E | W | `has_platform_privilege()` early-return gate; RLS not load-bearing (`20260310004215_orphaned_deletion_monitoring.sql:76`) | Retry a failed deletion workflow (admin) [admin-only] |
...
<!-- DO NOT HAND-EDIT — content between START/END is regenerated by
     frontend/scripts/gen-rpc-reachability-matrix.cjs. Rich
     "Current canonical body" column will be replaced with the simpler
     codegen-derived (name, bucket, reason) schema on first run. -->
...
<!-- DO NOT HAND-EDIT — content between START/END is regenerated by
     frontend/scripts/gen-rpc-reachability-matrix.cjs. The rich
     hand-curated per-table audit cluster below is preserved as the
     pre-codegen state. First codegen run replaces it with a simpler
     (name, bucket, reason) target table derived from @a4c-* tags.
     Per-table grouping is NOT @a4c-derivable; would need a follow-up
     enhancement (e.g., @a4c-rls-table tag). -->
...
- `organizations_projection` → 4 D RPCs (`get_organization_by_id`, `get_organization_direct_care_settings`, `get_organization_name`, `get_organization_details` [Stage R; subject to Phase 4 sub-audit § definer-bypasses-RLS]). Note: the 3 organization-lifecycle RPCs (`deactivate_organization`, `delete_organization`, `reactivate_organization`) moved to **E `[admin-only]`** per Stage R-6 F2 — `has_platform_privilege()` is the only enforcement, RLS not load-bearing. `safety_net_deactivate_organization` is also **E** (service-role-only Temporal compensation lever).
...
- `schedule_templates_projection` → 1 D RPC (`list_schedule_templates` [Stage R; subject to Phase 4 sub-audit § definer-bypasses-RLS])
