...
- **2026-04-30** — Migration `20260430172139_add_modify_user_roles_rpc.sql` ships **`api.modify_user_roles`** as the second multi-event Pattern A v2 RPC (after `update_role`). Last `candidate-for-extraction` from PR #33's ADR; closes the LB1–LB6 audit's extractable-Edge-Function-operations backlog. New behavior beyond `update_role`: a **partial-failure contract** — on mid-loop emit failure the RPC returns `{success: false, partial: true, error: 'PARTIAL_FAILURE', userId, addedRoleEventIds[], removedRoleEventIds[], failureIndex, failureSection, processingError}` rather than rolling back (which would destroy the `domain_events` audit rows the trigger already persisted with `processing_error`). Re-running with the same input arrays converges (handlers are idempotent on the projection unique key). Closes the legacy Edge Function path's revoke-side delegation gap by validating the **union** of adds + removes via `validate_role_assignment` (the legacy code only validated adds). `manage-user` Edge Function bumped v14 → **v15-modify-roles-extracted**.
