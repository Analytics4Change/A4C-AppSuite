...
- **2026-04-27** — Third extraction + scoped-permission retrofit: `manage-user delete` → `api.delete_user` (migration `20260427205333_extract_delete_user_rpc.sql`); plus retrofit of two prior extractions to the canonical scoped-permission pattern. Inventory row #10 moves `candidate` → `extracted`. `manage-user` Edge Function `DEPLOY_VERSION` bumps to `v13-delete-extracted`; `Operation` union narrows from `'deactivate' | 'reactivate' | 'delete' | 'modify_roles'` to `'deactivate' | 'reactivate' | 'modify_roles'`. **Architectural pivot**: the `software-architect-dbc` agent review (start of `manage-user-delete-to-sql-rpc/` planning) identified that PR #36 and PR #39 both broke the canonical scoped-permission pattern (already established in baseline_v4 at `bulk_assign_role` line 5498 and OU mutators 5940/6023) by mirroring the unscoped Edge Function helper at `_shared/types.ts:66-71`. Concrete attack vector: **intra-tenant cross-OU privilege escalation** — a delegated admin scoped to OU `acme.pediatrics` could act on users in `acme.cardiology`. The PR closes this for all three extracted user-targeted RPCs in one atomic story:
  - **New canonical helper** `public.get_user_target_path(p_user_id uuid, p_org_id uuid) → ltree` (migration `20260427203747`). Resolves `users.current_org_unit_id → organization_units_projection.path` with `organizations_projection.path` fallback. Includes **tenancy guard** that raises 42501 when `users.current_organization_id <> p_org_id` — closes the JWT/`switch_organization()` inconsistency window per baseline_v4:11738-11741 (latent today since org-switch frontend is unimplemented).
  - **`api.delete_user` scoped from inception**: `has_effective_permission('user.delete', get_user_target_path(...))`. Pattern A v2 read-back against `users.deleted_at`. Pre-emit idempotency guard returns success-false for already-deleted users (avoids audit-log noise).
  - **`api.update_user_notification_preferences` retrofit** (migration `20260427205449`): self-update bypass preserved (preferences are personal); admin branch now `has_effective_permission('user.update', get_user_target_path(...))`.
  - **`api.revoke_invitation` retrofit** (migration `20260427205549`): `has_effective_permission('user.create', organizations_projection.path)`. Also closes a latent **cross-tenant UUID-leak** — the prior existence check had no tenancy filter; an admin in tenant A who knew an invitation UUID belonging to tenant B could revoke it. Mismatched-tenancy now returns the same envelope as not-found (no UUID-existence leak).
  - **Pre-deploy regression check**: cross-OU query against `domain_events` (last 30 days, both retrofitted event types) returned **0 suspect calls**. All 3 unique callers held `provider_admin` with `scope_path = org root`; under scoped semantics, all 11 historical calls still pass.
  - **Raises the floor for the 2 remaining candidate extractions** (rows #9 `manage-user reactivate`, #11 `manage-user modify_roles`): (k) **scoped permissions are now canonical** — never use unscoped `has_permission()` for resource-scoped ops; use `has_effective_permission(<perm>, get_user_target_path(...))` for user-targeted RPCs. Codified as "Unscoped `has_permission()` is a code smell" rule in `infrastructure/supabase/CLAUDE.md` § Critical Rules. (l) The two-arg helper signature is future-proof against the org-switch JWT inconsistency window even though that window is currently unreachable. (m) Inventory of unscoped checks remaining: `_shared/types.ts:66-71` (TS helper), `manage-user/index.ts:225-241` (`modify_roles`, default `user.update`) — both will be retired when the corresponding RPCs land.
