...
Total: 16 operations across 7 functions. **12 load-bearing** + **0 candidate-for-extraction** + **4 extracted** (rows #7 invite-user revoke, #10 manage-user delete, #11 manage-user modify_roles, #12 manage-user update_notification_preferences; see Rollout history below). Inventory row #9 `manage-user reactivate` was reclassified `candidate-for-extraction` → `load-bearing` (LB1) on 2026-04-29 after PR #40 added an `auth.admin.updateUserById` unban call to the reactivate path. **The `candidate-for-extraction` backlog is empty as of PR #44 (2026-04-30).**

> **Classification audit note (2026-04-24)**: During PR #33 review, an audit of `manage-user reactivate` revealed no `auth.admin` call on its path (the ban/unban call at `manage-user/index.ts:719` is gated on `operation === 'deactivate'`). Row 9 was therefore moved from `load-bearing` (LB1) to `candidate-for-extraction`. The same audit uncovered a separate data-integrity finding: three user-lifecycle handlers referenced by `process_user_event` (`handle_user_deactivated`, `handle_user_reactivated`, `handle_user_deleted`) are missing from the repo — tracked separately at `dev/archived/fix-missing-user-lifecycle-handlers/` (resolved by PR #35).
>
> **Reclassification 2026-04-29 (PR #40)**: The candidate classification was REVERSED when PR #40 (`manage-user-delete-rpc-and-scope-retrofit`) added an `auth.admin.updateUserById(userId, { ban_duration: 'none' })` call to the reactivate path. The addition was driven by a separate security finding: deactivate's pre-existing `ban_duration: 'none'` bug (which UNBANS instead of bans) was empirically reproduced and fixed in PR #40 to use `ban_duration: '876000h'`; the reactivate path then needed a parallel unban call to restore login capability. With that addition, reactivate now invokes the Supabase Auth admin client and meets LB1 — extraction to a SQL RPC is no longer possible because PL/pgSQL cannot call `auth.admin.*`. Row 9 returns to `load-bearing` (LB1). The seed card `dev/archived/manage-user-reactivate-to-sql-rpc/` archived; future work targeting reactivate's silent-failure gap should be a Pattern A v2 retrofit within the Edge Function (analogous to the unseeded `manage-user-deactivate-pattern-a-v2-retrofit/`), not an extraction.
...
| 9 | `manage-user` | `reactivate` | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | **load-bearing** (LB1) | **Reclassified back to load-bearing 2026-04-29 (PR #40)** — PR #40 added `auth.admin.updateUserById(userId, { ban_duration: 'none' })` to the reactivate branch (lines around 519 in `manage-user/index.ts`) to clear the ban that the now-correctly-fixed deactivate path applies. Extraction to SQL RPC is no longer possible. Future work: Pattern A v2 retrofit within Edge Function. See Classification audit note above for the full reclassification trail (2026-04-24 candidate ← 2026-04-29 load-bearing). |
...
| 11 | ~~`manage-user`~~ | ~~`modify_roles`~~ | — | — | — | — | — | — | **extracted (2026-04-30)** | Moved to `api.modify_user_roles` SQL RPC. Fourth and final Edge→RPC extraction; second multi-event Pattern A v2 adopter (after `update_role`) with new partial-failure contract for mid-loop emit failures. Closes the legacy Edge Function path's revoke-side delegation gap by validating the union of adds + removes via `validate_role_assignment`. See migration `20260430172139_add_modify_user_roles_rpc.sql` and PR #44. Bundled with M3 type-level enforcement of helper choice (RPC-shape registry). |
...
| ~~Medium~~ | ~~`dev/active/manage-user-reactivate-to-sql-rpc/`~~ | ~~`manage-user reactivate`~~ | **Archived 2026-04-29** (`dev/archived/manage-user-reactivate-to-sql-rpc/`). Card's own Phase 0 gate fired: PR #40 added `auth.admin.updateUserById` unban to the reactivate path; row #9 reverted to LB1. Extraction is no longer possible. Future work: Pattern A v2 retrofit within Edge Function (analogous to unseeded deactivate retrofit). |
| ~~Low~~ | ~~`dev/archived/manage-user-modify-roles-to-sql-rpc/`~~ | ~~`manage-user modify_roles`~~ | **Extracted 2026-04-30** (PR #44) — see Rollout history. Final candidate retired; backlog now empty. |
...
- **2026-04-27** — Third extraction (`manage-user delete` → `api.delete_user`, migration `20260427205333_extract_delete_user_rpc.sql`) **plus same-day course correction**. Inventory row #10 moves `candidate` → `extracted`. `manage-user` Edge Function `DEPLOY_VERSION` bumps to `v13-delete-extracted`; `Operation` union narrows from `'deactivate' | 'reactivate' | 'delete' | 'modify_roles'` to `'deactivate' | 'reactivate' | 'modify_roles'`.
  ...
  **Raises the floor for the 2 remaining candidate extractions** (rows #9 `manage-user reactivate`, #11 `manage-user modify_roles`): (k) Choose between `has_permission` (resource has no path; pair with tenancy guard) and `has_effective_permission(perm, resource_path)` (resource has path AND permission can be derived via scope-bound implication). Documented in `infrastructure/supabase/CLAUDE.md` § Critical Rules. (l) `manage-user modify_roles` is **role-scoped** (acts on a `user_roles_projection` row that carries `scope_path`); it should use `has_effective_permission('user.role_assign', role_assignment.scope_path)`. (m) `manage-user reactivate` is identity-scoped (acts on the user, not on a path-bearing resource); it should follow the unscoped pattern of `delete_user`.
  ...
  > **⚠️ Note on (l) — superseded 2026-04-30 (PR #44)**: The role-scoped hypothesis was tested during the `modify_roles` extraction and rejected. The actual landed implementation is **unscoped + tenancy guard + delegate-per-role to `validate_role_assignment`** (which itself enforces template-scope containment via `check_scope_containment(role.org_hierarchy_scope, actor_scopes)`). The codified rule for choosing scoped vs unscoped (`infrastructure/supabase/CLAUDE.md` § Critical Rules, codified 2026-04-27 same-day after the PR #40 retrofit revert) is what prevailed: A4C user-identities have no organizational location finer than tenant, so user-targeted ops use unscoped + tenancy. Per-role delegation/scope enforcement happens inside `validate_role_assignment` on the union of adds + removes — closing the legacy Edge Function path's revoke-side gap (only adds were validated). See PR #44 Rollout entry below for the worked example (real OUs under `testorg-20260329`) that drove the planning-stage decision.
- **2026-04-29 (PR #41)** — Fourth Edge Function fix (NOT an extraction): `accept-invitation` `resolveInvitationPhonePlaceholder` helper closes a silent data-corruption bug (commits `8fd426ef..7dc0908c` on main). Frontend's `invitation-phone-N` placeholder phoneIds previously flowed through to `user.notification_preferences.updated` events, where the handler's `::UUID` cast failed silently and left newly-accepted users with `sms_enabled=false` regardless of inviter intent. Includes defense-in-depth (foreign UUIDs not in `createdPhoneIds` reject — closes a tenancy-boundary leak). DEPLOY_VERSION: `v17-phase6-phones-prefs` → `v19-phone-id-resolution-hardened` (v18 was a smoke-test iteration before architect review). Architect review (verdict APPROVE WITH MINOR FOLLOW-UPS) seeded 3 cards: `dev/parked/edge-function-deno-test-harness/`, `dev/active/fix-phone-emit-index-preservation/` (Issue 2 same bug class), `dev/parked/edge-function-ci-preview-deploy/` (Issue 5).
- **2026-04-29 (post-PR #40 hygiene)** — **Inventory row #9 (`manage-user reactivate`) reclassified `candidate-for-extraction` → `load-bearing` (LB1)**. PR #40 added an `auth.admin.updateUserById(userId, { ban_duration: 'none' })` unban call to the reactivate path (paired with the deactivate `ban_duration` security fix in the same PR). With that addition, reactivate now invokes the Supabase Auth admin client and meets LB1 — extraction to a SQL RPC is no longer possible because PL/pgSQL cannot call `auth.admin.*`. The seed card `dev/archived/manage-user-reactivate-to-sql-rpc/` archived; future work targeting reactivate's silent-failure gap should be a Pattern A v2 retrofit within the Edge Function, analogous to the unseeded `manage-user-deactivate-pattern-a-v2-retrofit/`. Inventory totals adjust: 11→12 load-bearing, 2→1 candidate-for-extraction, 3 extracted unchanged. Function-level tag for `manage-user` (partial-candidate) is unchanged since `modify_roles` (row #11) remains a candidate.

- **2026-04-30 (PR #44)** — **Fourth and final extraction** (`manage-user modify_roles` → `api.modify_user_roles`, migration `20260430172139_add_modify_user_roles_rpc.sql`). Inventory row #11 moves `candidate` → `extracted`. **The `candidate-for-extraction` backlog is now empty.** `manage-user` Edge Function `DEPLOY_VERSION` bumps to `v15-modify-roles-extracted`; `Operation` union narrows to `'deactivate' | 'reactivate'`. **Second multi-event Pattern A v2 adopter** after `update_role`: emits N × `user.role.revoked` then M × `user.role.assigned` from a single RPC body, captures all event_ids in `v_added_ids` / `v_removed_ids` arrays, reads back the projection state, surfaces aggregated `processing_error` from `domain_events` on miss. **New behavior: partial-failure contract** — on mid-loop emit failure the RPC returns `{success: false, partial: true, error: 'PARTIAL_FAILURE', userId, addedRoleEventIds, removedRoleEventIds, failureIndex, failureSection, processingError}` rather than rolling back (which would destroy `domain_events` audit rows the trigger already persisted). Re-running with the same input arrays converges (handlers idempotent on the projection unique key). **Closes the legacy Edge Function path's revoke-side delegation gap**: the legacy code passed only `roleIdsToAdd` to `validate_role_assignment`, leaving revokes unguarded; the new RPC validates the **union** of adds + removes via `array_cat(p_role_ids_to_add, p_role_ids_to_remove)`. Authorization model is unscoped + tenancy guard + delegate-per-role to `validate_role_assignment` (which itself enforces template-scope containment) — supersedes the planning-stage role-scoped hypothesis (see ⚠️ note above on bullet (l)). **Bundled with M3 type-level enforcement** (RPC-shape registry): every `api.*` RPC carries `COMMENT ON FUNCTION ... '@a4c-rpc-shape: envelope|read'`; codegen emits `EnvelopeRpcs` / `ReadRpcs` string-literal unions; helpers `apiRpc<T>` / `apiRpcEnvelope<T>` narrow `functionName` on those unions, making wrong-helper-for-shape a TypeScript compile error. CI workflow `.github/workflows/rpc-registry-sync.yml` runs against a locally-seeded `supabase start` container and fails on diff or `UncategorizedRpcs ≠ never`. **Raises the floor for the 0 remaining candidate extractions (none)**: (n) When introducing a new multi-event RPC, document the partial-failure contract explicitly — best-effort multi-event with idempotent re-run, NO transactional rollback (would destroy audit rows). (o) Every new `api.*` RPC migration MUST include a `COMMENT ON FUNCTION ... '@a4c-rpc-shape: envelope|read'` (see `infrastructure-guidelines/SKILL.md` Rule 17); DROP+CREATE with a signature change MUST re-issue the COMMENT in the same migration. (p) Wrong-helper-for-shape is a compile error: any future migration adding a write RPC will fail typecheck on the consumer side until both the migration's COMMENT lands and the registry is regenerated via `npm run gen:rpc-registry`.
