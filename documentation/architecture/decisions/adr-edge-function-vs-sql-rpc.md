...
Total: 16 operations across 7 functions. **12 load-bearing** + **1 candidate-for-extraction** + **3 extracted** (rows #7 invite-user revoke, #10 manage-user delete, #12 manage-user update_notification_preferences; see Rollout history below). Inventory row #9 `manage-user reactivate` was reclassified `candidate-for-extraction` → `load-bearing` (LB1) on 2026-04-29 after PR #40 added an `auth.admin.updateUserById` unban call to the reactivate path.
