...
- [ ] **RPC count re-verification** (paired to step 11): live `pg_proc` distinct count of `api.*` functions = **170** (matrix doc now mirrors per Stage R reconciliation 2026-05-29; previously stated 104 was a hand-curation undercount — see tasks.md § Reconciliation outcome). Identify any post-2026-05-29 new `api.*` RPCs lacking matrix classification: `git log --since=2026-05-29 -p -- infrastructure/supabase/supabase/migrations/ | grep -E "^\\+CREATE OR REPLACE FUNCTION \"api\""`. Each needs an explicit `@a4c-bucket` decision in the matrix doc BEFORE the step 11 migration drafts.
