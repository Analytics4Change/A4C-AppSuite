...
**Next action**: Continue Stage C drafting. Steps 1 + 2 + 3 done (with architect-review fold-in for 1+2). Next in sequence: Step 4 (`sync_accessible_organizations_from_grants` function + AFTER-INSERT/UPDATE/DELETE trigger on `cross_tenant_access_grants_projection`; predicate `status='active' AND (expires_at IS NULL OR expires_at > now())`). Then Step 5 (one-time backfill DO-block — idempotent dedup) → Step 6 (composite partial index closing the auth-hook query gap) → Step 7 (10 C-legacy RPC normalizations in plan.md constraint #7 / F2 ordering) → Step 8 (M3 re-tag) → Step 9 (CHECK constraint) → Step 10 (`access_grant.policy_override_applied` handler + perm-defined events) → Step 11 (170-RPC `@a4c-bucket` backfill). Steps 12/13 are file additions (codegen script + CI workflow) rather than SQL. Steps 14 + 15 close the migration. Architect review at a natural breakpoint (e.g., after Step 6 lands schema-side work, or after Step 8 closes the must-pair triad with Step 1+7) recommended per Stage R-6 fold-in precedent.
