...
**Next action**: Continue Stage C drafting. Steps 1+2+3+4+5 done with two rounds of architect-review fold-in already applied. Step 5 carried M1 forward via helper-based design (deviation from ADR sketch). Next in sequence: **Step 6** (composite partial index `idx_access_grants_consultant_user_status_partial ON cross_tenant_access_grants_projection (consultant_user_id, status) WHERE status='active'` — closes the auth-hook query gap; existing single-column partial index `idx_access_grants_consultant_user WHERE NOT NULL` does NOT cover the `status='active'` filter the auth hook will run; both indexes stay per plan.md N1) → Step 7 (10 C-legacy RPC normalizations in plan.md constraint #7 / F2 ordering: 5 OU mutators → 2 role-management mutations → 3 OU readers) → Step 8 (M3 re-tag for the 10 normalized RPCs + UncategorizedRpcs assertion) → Step 9 (CHECK constraint on authorization_type 5 values) → Step 10 (`access_grant.policy_override_applied` handler + perm-defined events for grant.create/view/revoke + 4 partner.* perms) → Step 11 (170-RPC `@a4c-bucket` backfill). Steps 12/13 are file additions (codegen script + CI workflow) rather than SQL. Steps 14+15 close the migration. Architect review at the next natural breakpoint (e.g., after Step 6+7+8 close the must-pair triad with Step 1) recommended.
