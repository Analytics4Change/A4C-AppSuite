...
**Next action**: Continue Stage C drafting. Steps 1+2+3+4+5+6 done with **three** rounds of architect-review fold-in already applied (Steps 1+2 fold-in 2026-06-01; Steps 3+4 fold-in 2026-06-01; **Steps 5+6 fold-in 2026-06-01** — folded F1 must-fix [Step 5 `consultant_org_id = ANY(u.accessible_organizations)` → `u.accessible_organizations @> ARRAY[g.consultant_org_id]::uuid[]` per codified CLAUDE.md canonical form; same drift class as the prior M1], F2 must-fix [Step 5 ↔ Step 4c predicate cohesion invariant comment], N1 nit [DDL-vs-DML transactional invariant explicit in Step 5 comment], N2 nit [Step 6 docblock sharpened — explicit bitmap-AND alternative reasoning vs vague "efficiently"], N3 nit [Step 5 pre-loop NOTICE emitting eligible count + drift detection in final NOTICE]). Schema-side schema/index work complete after Step 6. Next in sequence: **Step 7** (10 C-legacy RPC normalizations in plan.md constraint #7 / F2 ordering: 5 OU mutators → 2 role-management mutations → 3 OU readers; the operational-tripwire must-pair set that closes the DISTINCT ON tightening from Step 1) → Step 8 (M3 re-tag for the 10 normalized RPCs + `UncategorizedRpcs = never` assertion; closes the must-pair triad with Step 1+7) → Step 9 (CHECK constraint on `authorization_type` 5 values) → Step 10 (`access_grant.policy_override_applied` handler + perm-defined events for grant.create/view/revoke + 4 partner.* perms) → Step 11 (170-RPC `@a4c-bucket` backfill). Steps 12/13 are file additions (codegen script + CI workflow). Steps 14+15 close the migration. Per architect: Step 7 review should be dedicated pass (biggest-volume step; 10 RPC bodies + 10 COMMENT ON FUNCTION re-tags).
