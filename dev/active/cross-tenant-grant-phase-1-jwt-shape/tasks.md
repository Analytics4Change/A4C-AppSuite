...
**Next action**: Continue Stage C drafting. Steps 1 + 2 done; next in sequence: Step 3 (`custom_access_token_hook` rebase on `20260226002002_*.sql` body + `claims_version` bump to 5). Then Step 4 → 5 → 6 in order, then the Step 7 mutator/reader cluster, then 8 → 9 → 10 → 11. Steps 12/13 are file additions (codegen script + CI workflow) rather than SQL. Steps 14 + 15 close the migration.
