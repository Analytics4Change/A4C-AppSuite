...
**Surfaced during implementation**: `handle_user_profile_updated` is referenced in `process_user_event()` router but does NOT exist in the live DB. Every call to `api.update_user` currently triggers an `EXCEPTION WHEN OTHERS` in the dispatcher and sets `processing_error` (caught silently pre-migration). With the read-back in place, the silent failure is no longer silent — the RPC will return `{success: true, user: <stale row>}` because the `users` row pre-exists from signup. This is a pre-existing bug NOT in scope for this migration; track separately as a follow-up to either (a) create the missing handler or (b) deprecate `api.update_user` if no longer needed.
