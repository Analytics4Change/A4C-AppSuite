...
| L6 | `create_access_grant` emergency_access + NULL auth_ref | 🚨 **BLOCKED → ✅ FIXED via PR #79** | Original block: `grant_role_template_name` required but only `(var_default, var_contract)` seeded. Fix (PR #79, merge `ba7fcaf4`, 2026-06-22): seeded `emergency_default` template `{client.view, medication.view}` (Option B) + bounded emergency expiry (NULL → 22004, >72h → 22023). L6 re-probe post-fix (transactional simulate-JWT, rolled back): happy path `success:true` with `[{client.view},{medication.view}]` @ `liveforlife.aspen`; both expiry guards fire; `permission_overrides={medication.view}` narrows via INTERSECT. **L6 now PASS.** |
