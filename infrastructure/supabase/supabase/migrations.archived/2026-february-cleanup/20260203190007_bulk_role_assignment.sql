-- =============================================================================
-- Migration: Bulk Role Assignment API Functions
-- Purpose: Enable assigning multiple users to a role in a single operation
-- =============================================================================

-- -----------------------------------------------------------------------------
-- Function: api.list_users_for_bulk_assignment
-- Purpose: List users eligible for bulk assignment to a specific role
-- Returns users not already assigned to the role at the specified scope
-- -----------------------------------------------------------------------------
...
COMMENT ON FUNCTION "api"."list_users_for_bulk_assignment"(UUID, extensions.ltree, TEXT, INT, INT) IS
'List users eligible for bulk role assignment.
...
-- -----------------------------------------------------------------------------
-- Function: api.bulk_assign_role
-- Purpose: Assign multiple users to a role in a single operation
-- Emits user.role.assigned event for each successful assignment
-- Uses correlation_id to link all events from the same bulk operation
-- -----------------------------------------------------------------------------
...
COMMENT ON FUNCTION "api"."bulk_assign_role"(UUID, UUID[], extensions.ltree, UUID, TEXT) IS
'Assign multiple users to a role in a single operation.
Requires user.role_assign permission at the specified scope.
Emits user.role.assigned event for each successful assignment.
...
GRANT EXECUTE ON FUNCTION "api"."list_users_for_bulk_assignment"(UUID, extensions.ltree, TEXT, INT, INT) TO authenticated;
GRANT EXECUTE ON FUNCTION "api"."bulk_assign_role"(UUID, UUID[], extensions.ltree, UUID, TEXT) TO authenticated;
