-- Migration: OU Array Append Fix
-- Date: 2025-12-24
-- Description: Fixes "malformed array literal" error in api.update_organization_unit
--
-- Root Cause:
--   The || operator with TEXT[] and a string literal is ambiguous.
--   PostgreSQL interprets 'display_name' as an array literal to parse,
--   not as a single text element to append.
--
-- Fix:
--   Replace: v_updated_fields := v_updated_fields || 'field_name'
--   With:    v_updated_fields := array_append(v_updated_fields, 'field_name')
--
-- This explicitly tells PostgreSQL to append a single text element to the array.
--
-- Pattern Reference:
--   - RPC function validates and builds event data
--   - RPC function INSERTs into domain_events
--   - Database trigger fires process_domain_event()
--   - Event processor updates projection tables
--   See: documentation/infrastructure/guides/supabase/docs/EVENT-DRIVEN-ARCHITECTURE.md
