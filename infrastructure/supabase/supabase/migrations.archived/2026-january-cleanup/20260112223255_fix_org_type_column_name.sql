-- Fix org_type column name bug in process_organization_event function
--
-- Bug: Migration 20260109020002_business_scoped_correlation_id.sql line 78
-- used 'org_type' as column name, but organizations_projection has 'type'
--
  ...
  CASE p_event.event_type
    ...
    WHEN 'organization.created' THEN
      ...
      INSERT INTO organizations_projection (
        id, name, display_name, slug, type, path, parent_path, parent_id,
...
COMMENT ON FUNCTION public.process_organization_event(record) IS
  'Organization event processor with business-scoped correlation_id support for invitation lifecycle tracing. Fixed: uses correct column name "type" instead of "org_type".';
