-- Migration: Remove backwards compatibility COALESCE for camelCase
--
-- All existing notification_preferences events have been processed.
-- Frontend now emits snake_case per AsyncAPI contract.
-- Remove camelCase fallbacks (phoneId, inApp) to enforce contract compliance.
...
BEGIN
  CASE p_event.event_type
    ...
    -- NO BACKWARDS COMPATIBILITY: camelCase removed (all events migrated)
    -- =========================================================================
...
AsyncAPI Compliance (2026-01-19):
- notification_preferences handler uses snake_case per contract
- phone_id, in_app (camelCase fallbacks removed after migration)
...
- 2026-01-19: Aligned with AsyncAPI contract, added camelCase backwards compat
- 2026-01-19: Removed camelCase backwards compat (all events migrated)';
