...
-- correlation_id ties together the ENTIRE business transaction lifecycle, not just
-- a single request. This enables complete lifecycle tracing for audit and debugging.
--
-- For invitations, the correlation_id is:
-- 1. Generated when invitation is created (user.invited event)
-- 2. Stored in invitations_projection
-- 3. Looked up and reused for all subsequent events (resent, revoked, accepted, expired)
--
-- This migration:
-- A) Adds correlation_id column to invitations_projection
-- B) Updates process_organization_event to store correlation_id from event metadata
-- C) Updates get_invitation_by_token to return correlation_id

-- ============================================================================
-- Part A: Add correlation_id column to invitations_projection
-- ============================================================================

ALTER TABLE public.invitations_projection
ADD COLUMN IF NOT EXISTS correlation_id UUID;

COMMENT ON COLUMN public.invitations_projection.correlation_id IS
  'Business-scoped correlation ID for complete lifecycle tracing. Generated at invitation creation, reused for resend/revoke/accept/expire events.';

-- Index for efficient correlation_id lookups
CREATE INDEX IF NOT EXISTS idx_invitations_projection_correlation_id
ON public.invitations_projection (correlation_id)
WHERE correlation_id IS NOT NULL;

-- ============================================================================
-- Part B: Update process_organization_event to store correlation_id
-- ============================================================================
...
COMMENT ON FUNCTION public.process_organization_event(record) IS
  'Organization event processor with business-scoped correlation_id support for invitation lifecycle tracing';

-- ============================================================================
-- Part C: Update get_invitation_by_token to return correlation_id
-- ============================================================================
...
RETURNS TABLE (
  ...
  correlation_id UUID  -- Added for lifecycle tracing
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, extensions, pg_temp
...
BEGIN
  ...
  SELECT
    ...
    i.expires_at,
    i.accepted_at,
    i.correlation_id  -- Return for lifecycle tracing
...
COMMENT ON FUNCTION api.get_invitation_by_token(TEXT) IS
  'Get invitation details by token for validation. Returns correlation_id for business-scoped lifecycle tracing. Handles both legacy role column and new roles JSONB array.';
