...
COMMENT ON SCHEMA "api" IS 'API functions for frontend RPC calls.
...
COMMENT ON SCHEMA "public" IS 'Multi-Role Authorization v3 RLS Helpers:
...
CREATE OR REPLACE FUNCTION "api"."assign_client_to_user"("p_user_id" "uuid", "p_client_id" "uuid", "p_assigned_until" timestamp with time zone DEFAULT NULL::timestamp with time zone, "p_notes" "text" DEFAULT NULL::"text", "p_reason" "text" DEFAULT NULL::"text") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."assign_client_to_user"("p_user_id" "uuid", "p_client_id" "uuid", "p_assigned_until" timestamp with time zone, "p_notes" "text", "p_reason" "text") OWNER TO "postgres";
...
ALTER FUNCTION "api"."bulk_assign_role"("p_role_id" "uuid", "p_user_ids" "uuid"[], "p_scope_path" "extensions"."ltree", "p_correlation_id" "uuid", "p_reason" "text") OWNER TO "postgres";


COMMENT ON FUNCTION "api"."bulk_assign_role"("p_role_id" "uuid", "p_user_ids" "uuid"[], "p_scope_path" "extensions"."ltree", "p_correlation_id" "uuid", "p_reason" "text") IS 'Assign multiple users to a role in a single operation.
Requires user.role_assign permission at the specified scope.
Emits user.role.assigned event for each successful assignment.
All events from the same bulk operation are linked via correlation_id.
Returns detailed result with successful and failed assignments.';
...
COMMENT ON FUNCTION "api"."create_organization_unit"("p_parent_id" "uuid", "p_name" "text", "p_display_name" "text", "p_timezone" "text") IS 'Create a new organization unit (CQRS via domain events).
...
CREATE OR REPLACE FUNCTION "api"."create_user_schedule"("p_user_id" "uuid", "p_schedule" "jsonb", "p_org_unit_id" "uuid" DEFAULT NULL::"uuid", "p_effective_from" "date" DEFAULT NULL::"date", "p_effective_until" "date" DEFAULT NULL::"date", "p_reason" "text" DEFAULT NULL::"text") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."create_user_schedule"("p_user_id" "uuid", "p_schedule" "jsonb", "p_org_unit_id" "uuid", "p_effective_from" "date", "p_effective_until" "date", "p_reason" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "api"."create_user_schedule"("p_user_id" "uuid", "p_schedule_name" "text", "p_schedule" "jsonb", "p_org_unit_id" "uuid" DEFAULT NULL::"uuid", "p_effective_from" "date" DEFAULT NULL::"date", "p_effective_until" "date" DEFAULT NULL::"date", "p_reason" "text" DEFAULT NULL::"text") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."create_user_schedule"("p_user_id" "uuid", "p_schedule_name" "text", "p_schedule" "jsonb", "p_org_unit_id" "uuid", "p_effective_from" "date", "p_effective_until" "date", "p_reason" "text") OWNER TO "postgres";
...
COMMENT ON FUNCTION "api"."deactivate_organization_unit"("p_unit_id" "uuid") IS 'Deactivate an organization unit with cascade to descendants (CQRS via domain events).
...
CREATE OR REPLACE FUNCTION "api"."deactivate_user_schedule"("p_schedule_id" "uuid", "p_reason" "text" DEFAULT NULL::"text") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."deactivate_user_schedule"("p_schedule_id" "uuid", "p_reason" "text") OWNER TO "postgres";
...
COMMENT ON FUNCTION "api"."delete_organization_unit"("p_unit_id" "uuid") IS 'Soft-delete an organization unit (CQRS via domain events).
...
CREATE OR REPLACE FUNCTION "api"."delete_user_schedule"("p_schedule_id" "uuid", "p_reason" "text" DEFAULT NULL::"text") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."delete_user_schedule"("p_schedule_id" "uuid", "p_reason" "text") OWNER TO "postgres";
...
COMMENT ON FUNCTION "api"."emit_domain_event"("p_stream_id" "uuid", "p_stream_type" "text", "p_event_type" "text", "p_event_data" "jsonb", "p_event_metadata" "jsonb") IS 'Emit domain event with auto-calculated stream_version, tracing support, and automatic fallback to PostgREST pre-request hook session variables when metadata fields are NULL. Explicit metadata always takes precedence.';
...
CREATE OR REPLACE FUNCTION "api"."get_current_org_unit"() RETURNS TABLE("id" "uuid", "name" "text", "path" "extensions"."ltree", "organization_id" "uuid")
...
ALTER FUNCTION "api"."get_current_org_unit"() OWNER TO "postgres";


COMMENT ON FUNCTION "api"."get_current_org_unit"() IS 'Get the current user''s selected org unit context.
...
CREATE OR REPLACE FUNCTION "api"."get_organization_direct_care_settings"("p_org_id" "uuid") RETURNS "jsonb"
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
...
ALTER FUNCTION "api"."get_organization_direct_care_settings"("p_org_id" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "api"."get_organization_direct_care_settings"("p_org_id" "uuid") IS 'Get direct care feature flags for an organization.
...
COMMENT ON FUNCTION "api"."get_organization_unit_by_id"("p_unit_id" "uuid") IS 'Get a single organization unit by ID.
...
COMMENT ON FUNCTION "api"."get_organization_unit_descendants"("p_unit_id" "uuid") IS 'Get all descendants of an organizational unit.
Uses get_permission_scope(organization.view_ou) for authorization (claims v4).';
...
COMMENT ON FUNCTION "api"."get_organization_units"("p_status" "text", "p_search_term" "text") IS 'List all organization units within user scope.
Uses get_permission_scope(organization.view_ou) for authorization (claims v4).
...
CREATE OR REPLACE FUNCTION "api"."get_role_by_name"("p_org_id" "uuid", "p_role_name" "text") RETURNS TABLE("id" "uuid", "name" "text", "organization_id" "uuid")
  ...
  ORDER BY r.organization_id DESC NULLS LAST
  LIMIT 1;
$$;
...
CREATE OR REPLACE FUNCTION "api"."get_schedule_by_id"("p_schedule_id" "uuid") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."get_schedule_by_id"("p_schedule_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "api"."list_user_client_assignments"("p_org_id" "uuid" DEFAULT NULL::"uuid", "p_user_id" "uuid" DEFAULT NULL::"uuid", "p_client_id" "uuid" DEFAULT NULL::"uuid", "p_active_only" boolean DEFAULT true) RETURNS "jsonb"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."list_user_client_assignments"("p_org_id" "uuid", "p_user_id" "uuid", "p_client_id" "uuid", "p_active_only" boolean) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "api"."list_user_schedules"("p_org_id" "uuid" DEFAULT NULL::"uuid", "p_user_id" "uuid" DEFAULT NULL::"uuid", "p_org_unit_id" "uuid" DEFAULT NULL::"uuid", "p_active_only" boolean DEFAULT true) RETURNS "jsonb"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."list_user_schedules"("p_org_id" "uuid", "p_user_id" "uuid", "p_org_unit_id" "uuid", "p_active_only" boolean) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "api"."list_user_schedules"("p_org_id" "uuid" DEFAULT NULL::"uuid", "p_user_id" "uuid" DEFAULT NULL::"uuid", "p_org_unit_id" "uuid" DEFAULT NULL::"uuid", "p_active_only" boolean DEFAULT true, "p_schedule_name" "text" DEFAULT NULL::"text") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."list_user_schedules"("p_org_id" "uuid", "p_user_id" "uuid", "p_org_unit_id" "uuid", "p_active_only" boolean, "p_schedule_name" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "api"."list_users_for_bulk_assignment"("p_role_id" "uuid", "p_scope_path" "extensions"."ltree", "p_search_term" "text" DEFAULT NULL::"text", "p_limit" integer DEFAULT 100, "p_offset" integer DEFAULT 0) RETURNS TABLE("id" "uuid", "email" "text", "display_name" "text", "is_active" boolean, "current_roles" "text"[], "is_already_assigned" boolean)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."list_users_for_bulk_assignment"("p_role_id" "uuid", "p_scope_path" "extensions"."ltree", "p_search_term" "text", "p_limit" integer, "p_offset" integer) OWNER TO "postgres";


COMMENT ON FUNCTION "api"."list_users_for_bulk_assignment"("p_role_id" "uuid", "p_scope_path" "extensions"."ltree", "p_search_term" "text", "p_limit" integer, "p_offset" integer) IS 'List users eligible for bulk role assignment.
Requires user.role_assign permission at the specified scope.
Returns users with their current roles and whether they are already assigned to the target role.
Already-assigned users are sorted to the end of the list.';



CREATE OR REPLACE FUNCTION "api"."list_users_for_role_management"("p_role_id" "uuid", "p_scope_path" "extensions"."ltree", "p_search_term" "text" DEFAULT NULL::"text", "p_limit" integer DEFAULT 100, "p_offset" integer DEFAULT 0) RETURNS TABLE("id" "uuid", "email" "text", "display_name" "text", "is_active" boolean, "current_roles" "text"[], "is_assigned" boolean)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."list_users_for_role_management"("p_role_id" "uuid", "p_scope_path" "extensions"."ltree", "p_search_term" "text", "p_limit" integer, "p_offset" integer) OWNER TO "postgres";
...
COMMENT ON FUNCTION "api"."reactivate_organization_unit"("p_unit_id" "uuid") IS 'Reactivate an organization unit with cascade to descendants (CQRS via domain events).
Uses get_permission_scope(organization.update_ou) for authorization (claims v4).';
...
CREATE OR REPLACE FUNCTION "api"."reactivate_user_schedule"("p_schedule_id" "uuid", "p_reason" "text" DEFAULT NULL::"text") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."reactivate_user_schedule"("p_schedule_id" "uuid", "p_reason" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "api"."switch_org_unit"("p_org_unit_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."switch_org_unit"("p_org_unit_id" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "api"."switch_org_unit"("p_org_unit_id" "uuid") IS 'Switch the current user''s working org unit context.
...
ALTER FUNCTION "api"."sync_role_assignments"("p_role_id" "uuid", "p_user_ids_to_add" "uuid"[], "p_user_ids_to_remove" "uuid"[], "p_scope_path" "extensions"."ltree", "p_correlation_id" "uuid", "p_reason" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "api"."unassign_client_from_user"("p_user_id" "uuid", "p_client_id" "uuid", "p_reason" "text" DEFAULT NULL::"text") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."unassign_client_from_user"("p_user_id" "uuid", "p_client_id" "uuid", "p_reason" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "api"."update_organization_direct_care_settings"("p_org_id" "uuid", "p_enable_staff_client_mapping" boolean DEFAULT NULL::boolean, "p_enable_schedule_enforcement" boolean DEFAULT NULL::boolean) RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."update_organization_direct_care_settings"("p_org_id" "uuid", "p_enable_staff_client_mapping" boolean, "p_enable_schedule_enforcement" boolean) OWNER TO "postgres";


COMMENT ON FUNCTION "api"."update_organization_direct_care_settings"("p_org_id" "uuid", "p_enable_staff_client_mapping" boolean, "p_enable_schedule_enforcement" boolean) IS 'Update direct care feature flags for an organization.
...
CREATE OR REPLACE FUNCTION "api"."update_organization_direct_care_settings"("p_org_id" "uuid", "p_enable_staff_client_mapping" boolean DEFAULT NULL::boolean, "p_enable_schedule_enforcement" boolean DEFAULT NULL::boolean, "p_reason" "text" DEFAULT NULL::"text") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."update_organization_direct_care_settings"("p_org_id" "uuid", "p_enable_staff_client_mapping" boolean, "p_enable_schedule_enforcement" boolean, "p_reason" "text") OWNER TO "postgres";


COMMENT ON FUNCTION "api"."update_organization_direct_care_settings"("p_org_id" "uuid", "p_enable_staff_client_mapping" boolean, "p_enable_schedule_enforcement" boolean, "p_reason" "text") IS 'Update direct care feature flags for an organization.
...
COMMENT ON FUNCTION "api"."update_organization_unit"("p_unit_id" "uuid", "p_name" "text", "p_display_name" "text", "p_timezone" "text") IS 'Update an organization unit (CQRS via domain events).
Uses get_permission_scope(organization.update_ou) for authorization (claims v4).';
...
CREATE OR REPLACE FUNCTION "api"."update_user_schedule"("p_schedule_id" "uuid", "p_schedule" "jsonb" DEFAULT NULL::"jsonb", "p_org_unit_id" "uuid" DEFAULT NULL::"uuid", "p_effective_from" "date" DEFAULT NULL::"date", "p_effective_until" "date" DEFAULT NULL::"date", "p_reason" "text" DEFAULT NULL::"text") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."update_user_schedule"("p_schedule_id" "uuid", "p_schedule" "jsonb", "p_org_unit_id" "uuid", "p_effective_from" "date", "p_effective_until" "date", "p_reason" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "api"."update_user_schedule"("p_schedule_id" "uuid", "p_schedule_name" "text" DEFAULT NULL::"text", "p_schedule" "jsonb" DEFAULT NULL::"jsonb", "p_org_unit_id" "uuid" DEFAULT NULL::"uuid", "p_effective_from" "date" DEFAULT NULL::"date", "p_effective_until" "date" DEFAULT NULL::"date", "p_reason" "text" DEFAULT NULL::"text") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "api"."update_user_schedule"("p_schedule_id" "uuid", "p_schedule_name" "text", "p_schedule" "jsonb", "p_org_unit_id" "uuid", "p_effective_from" "date", "p_effective_until" "date", "p_reason" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."compute_effective_permissions"("p_user_id" "uuid", "p_org_id" "uuid") RETURNS TABLE("permission_name" "text", "effective_scope" "extensions"."ltree")
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
WITH
...
ALTER FUNCTION "public"."compute_effective_permissions"("p_user_id" "uuid", "p_org_id" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."compute_effective_permissions"("p_user_id" "uuid", "p_org_id" "uuid") IS 'Computes effective permissions for a user within an organization.
...
COMMENT ON FUNCTION "public"."custom_access_token_hook"("event" "jsonb") IS 'JWT custom claims hook v4 - effective permissions only.
...
CREATE OR REPLACE FUNCTION "public"."get_clients_assigned_to_user"("p_user_id" "uuid", "p_org_id" "uuid" DEFAULT NULL::"uuid") RETURNS TABLE("client_id" "uuid", "assigned_at" timestamp with time zone, "notes" "text")
    LANGUAGE "sql" STABLE
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
...
ALTER FUNCTION "public"."get_clients_assigned_to_user"("p_user_id" "uuid", "p_org_id" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_clients_assigned_to_user"("p_user_id" "uuid", "p_org_id" "uuid") IS 'Get all clients currently assigned to a user.
...
CREATE OR REPLACE FUNCTION "public"."get_permission_scope"("p_permission" "text") RETURNS "extensions"."ltree"
    LANGUAGE "sql" STABLE
    SET "search_path" TO 'public', 'extensions'
    AS $$
  SELECT (ep->>'s')::ltree
...
ALTER FUNCTION "public"."get_permission_scope"("p_permission" "text") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_permission_scope"("p_permission" "text") IS 'Get the scope at which the user has a specific permission.
...
CREATE OR REPLACE FUNCTION "public"."get_staff_assigned_to_client"("p_client_id" "uuid", "p_org_id" "uuid" DEFAULT NULL::"uuid") RETURNS TABLE("user_id" "uuid", "assigned_at" timestamp with time zone, "notes" "text")
    LANGUAGE "sql" STABLE
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
...
ALTER FUNCTION "public"."get_staff_assigned_to_client"("p_client_id" "uuid", "p_org_id" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_staff_assigned_to_client"("p_client_id" "uuid", "p_org_id" "uuid") IS 'Get all staff currently assigned to a client.
...
CREATE OR REPLACE FUNCTION "public"."handle_organization_activated"("p_event" "record") RETURNS "void"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."handle_organization_activated"("p_event" "record") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."handle_organization_direct_care_settings_updated"("p_event" "record") RETURNS "void"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."handle_organization_direct_care_settings_updated"("p_event" "record") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."handle_organization_direct_care_settings_updated"("p_event" "record") IS 'Event handler for organization.direct_care_settings.updated events.
...
CREATE OR REPLACE FUNCTION "public"."handle_permission_updated"("p_event" "record") RETURNS "void"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."handle_permission_updated"("p_event" "record") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."handle_permission_updated"("p_event" "record") IS 'Event handler for permission.updated events.
...
CREATE OR REPLACE FUNCTION "public"."handle_user_client_assigned"("p_event" "record") RETURNS "void"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."handle_user_client_assigned"("p_event" "record") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."handle_user_client_assigned"("p_event" "record") IS 'Event handler for user.client.assigned events.
Route from process_user_event: WHEN ''user.client.assigned'' THEN PERFORM handle_user_client_assigned(NEW);';



CREATE OR REPLACE FUNCTION "public"."handle_user_client_unassigned"("p_event" "record") RETURNS "void"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."handle_user_client_unassigned"("p_event" "record") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."handle_user_client_unassigned"("p_event" "record") IS 'Event handler for user.client.unassigned events.
...
CREATE OR REPLACE FUNCTION "public"."handle_user_schedule_created"("p_event" "record") RETURNS "void"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."handle_user_schedule_created"("p_event" "record") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."handle_user_schedule_created"("p_event" "record") IS 'Event handler for user.schedule.created events.
Inserts a new schedule row into user_schedule_policies_projection.
Idempotent via ON CONFLICT (id) DO NOTHING.';



CREATE OR REPLACE FUNCTION "public"."handle_user_schedule_deactivated"("p_event" "record") RETURNS "void"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."handle_user_schedule_deactivated"("p_event" "record") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."handle_user_schedule_deactivated"("p_event" "record") IS 'Event handler for user.schedule.deactivated events.
Sets is_active = false on matched schedule row.';



CREATE OR REPLACE FUNCTION "public"."handle_user_schedule_deleted"("p_event" "record") RETURNS "void"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."handle_user_schedule_deleted"("p_event" "record") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."handle_user_schedule_deleted"("p_event" "record") IS 'Event handler for user.schedule.deleted events.
Permanently removes inactive schedule row from projection.
Only deletes if is_active = false (safety guard).';



CREATE OR REPLACE FUNCTION "public"."handle_user_schedule_reactivated"("p_event" "record") RETURNS "void"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."handle_user_schedule_reactivated"("p_event" "record") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."handle_user_schedule_reactivated"("p_event" "record") IS 'Event handler for user.schedule.reactivated events.
Sets is_active = true on matched schedule row.';



CREATE OR REPLACE FUNCTION "public"."handle_user_schedule_updated"("p_event" "record") RETURNS "void"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."handle_user_schedule_updated"("p_event" "record") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."handle_user_schedule_updated"("p_event" "record") IS 'Event handler for user.schedule.updated events.
...
CREATE OR REPLACE FUNCTION "public"."has_effective_permission"("p_permission" "text", "p_target_path" "extensions"."ltree") RETURNS boolean
    LANGUAGE "sql" STABLE
    SET "search_path" TO 'public', 'extensions'
    AS $$
  SELECT EXISTS (
...
ALTER FUNCTION "public"."has_effective_permission"("p_permission" "text", "p_target_path" "extensions"."ltree") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."has_effective_permission"("p_permission" "text", "p_target_path" "extensions"."ltree") IS 'Check if the current user has a permission at or above the target scope path.
...
COMMENT ON FUNCTION "public"."has_org_admin_permission"() IS 'Check if user has organization admin permissions via effective_permissions JWT claim (claims v4).
...
COMMENT ON FUNCTION "public"."has_permission"("p_permission" "text") IS 'Check if the current user has a permission (without scope check).
...
COMMENT ON FUNCTION "public"."has_platform_privilege"() IS 'Check if current user has platform.admin permission.
...
CREATE OR REPLACE FUNCTION "public"."is_user_assigned_to_client"("p_user_id" "uuid", "p_client_id" "uuid") RETURNS boolean
    LANGUAGE "sql" STABLE
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
  SELECT EXISTS (
...
ALTER FUNCTION "public"."is_user_assigned_to_client"("p_user_id" "uuid", "p_client_id" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."is_user_assigned_to_client"("p_user_id" "uuid", "p_client_id" "uuid") IS 'Check if a user is currently assigned to a client.
...
CREATE OR REPLACE FUNCTION "public"."is_user_on_schedule"("p_user_id" "uuid", "p_org_id" "uuid", "p_org_unit_id" "uuid" DEFAULT NULL::"uuid", "p_check_time" timestamp with time zone DEFAULT "now"()) RETURNS boolean
    LANGUAGE "plpgsql" STABLE
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."is_user_on_schedule"("p_user_id" "uuid", "p_org_id" "uuid", "p_org_unit_id" "uuid", "p_check_time" timestamp with time zone) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."is_user_on_schedule"("p_user_id" "uuid", "p_org_id" "uuid", "p_org_unit_id" "uuid", "p_check_time" timestamp with time zone) IS 'Check if a user is currently within their scheduled hours.
...
CREATE OR REPLACE FUNCTION "public"."postgrest_pre_request"() RETURNS "void"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."postgrest_pre_request"() OWNER TO "postgres";


COMMENT ON FUNCTION "public"."postgrest_pre_request"() IS 'PostgREST pre-request hook: extracts tracing headers (X-Correlation-ID, traceparent, X-Session-ID) into app.* session variables for automatic event tracing. Fail-open: errors are silently ignored.';
...
COMMENT ON FUNCTION "public"."process_organization_event"("p_event" "record") IS 'Organization event router v3 - dispatches to individual handler functions.
...
COMMENT ON FUNCTION "public"."process_rbac_event"("p_event" "record") IS 'RBAC event router v3 - dispatches to individual handler functions.
...
COMMENT ON FUNCTION "public"."process_user_event"("p_event" "record") IS 'User event router v7 - dispatches to individual handler functions.
...
COMMENT ON FUNCTION "public"."uuid_generate_v7"() IS 'Generates RFC 9562 UUIDv7 - time-ordered UUID with millisecond precision';
...
COMMENT ON COLUMN "public"."users"."current_org_unit_id" IS 'Currently selected org unit context for user-centric workflows.
...
CREATE TABLE IF NOT EXISTS "public"."invitations_projection" (
    ...
    CONSTRAINT "chk_invitation_status" CHECK (("status" = ANY (ARRAY['pending'::"text", 'accepted'::"text", 'expired'::"text", 'deleted'::"text", 'revoked'::"text"]))),
...
CREATE TABLE IF NOT EXISTS "public"."organizations_projection" (
    ...
    "direct_care_settings" "jsonb" DEFAULT '{"enable_schedule_enforcement": false, "enable_staff_client_mapping": false}'::"jsonb",
...
COMMENT ON COLUMN "public"."organizations_projection"."direct_care_settings" IS 'Feature flags for direct care workflow routing:
...
CREATE TABLE IF NOT EXISTS "public"."permission_implications" (
    ...
    "implies_permission_id" "uuid" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "permission_implications_check" CHECK (("permission_id" <> "implies_permission_id"))
);


ALTER TABLE "public"."permission_implications" OWNER TO "postgres";


COMMENT ON TABLE "public"."permission_implications" IS 'Defines permission implication rules. If permission A implies permission B,
...
COMMENT ON COLUMN "public"."permission_implications"."permission_id" IS 'The permission that grants (implies) another permission';



COMMENT ON COLUMN "public"."permission_implications"."implies_permission_id" IS 'The permission that is implied (granted) by the other permission';
...
CREATE TABLE IF NOT EXISTS "public"."user_client_assignments_projection" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "user_id" "uuid" NOT NULL,
...
ALTER TABLE "public"."user_client_assignments_projection" OWNER TO "postgres";


COMMENT ON TABLE "public"."user_client_assignments_projection" IS 'CQRS projection of user.client.* events - stores staff-to-client assignments.
...
COMMENT ON COLUMN "public"."user_client_assignments_projection"."client_id" IS 'Client being assigned to this staff member.
Note: No FK constraint yet - clients table will be created in a future migration.';



COMMENT ON COLUMN "public"."user_client_assignments_projection"."assigned_until" IS 'Optional end date for temporary assignments.
...
ALTER TABLE ONLY "public"."user_roles_projection" REPLICA IDENTITY FULL;
...
CREATE TABLE IF NOT EXISTS "public"."user_schedule_policies_projection" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "user_id" "uuid" NOT NULL,
    "organization_id" "uuid" NOT NULL,
...
ALTER TABLE "public"."user_schedule_policies_projection" OWNER TO "postgres";


COMMENT ON TABLE "public"."user_schedule_policies_projection" IS 'CQRS projection of user.schedule.* events - stores recurring weekly schedules.
...
COMMENT ON COLUMN "public"."user_schedule_policies_projection"."schedule" IS 'Weekly recurring schedule pattern as JSONB.
...
COMMENT ON COLUMN "public"."user_schedule_policies_projection"."org_unit_id" IS 'Optional OU scope for this schedule.
...
ALTER TABLE ONLY "public"."contact_addresses"
    ADD CONSTRAINT "contact_addresses_pkey" PRIMARY KEY ("contact_id", "address_id");
...
ALTER TABLE ONLY "public"."contact_phones"
    ADD CONSTRAINT "contact_phones_pkey" PRIMARY KEY ("contact_id", "phone_id");
...
ALTER TABLE ONLY "public"."organization_addresses"
    ADD CONSTRAINT "organization_addresses_pkey" PRIMARY KEY ("organization_id", "address_id");
...
ALTER TABLE ONLY "public"."organization_contacts"
    ADD CONSTRAINT "organization_contacts_pkey" PRIMARY KEY ("organization_id", "contact_id");
...
ALTER TABLE ONLY "public"."organization_phones"
    ADD CONSTRAINT "organization_phones_pkey" PRIMARY KEY ("organization_id", "phone_id");
...
ALTER TABLE ONLY "public"."permission_implications"
    ADD CONSTRAINT "permission_implications_pkey" PRIMARY KEY ("permission_id", "implies_permission_id");
...
ALTER TABLE ONLY "public"."phone_addresses"
    ADD CONSTRAINT "phone_addresses_pkey" PRIMARY KEY ("phone_id", "address_id");
...
ALTER TABLE ONLY "public"."user_client_assignments_projection"
    ADD CONSTRAINT "user_client_assignments_projection_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."user_client_assignments_projection"
    ADD CONSTRAINT "user_client_assignments_unique" UNIQUE ("user_id", "client_id");
...
ALTER TABLE ONLY "public"."user_roles_projection"
    ADD CONSTRAINT "user_roles_projection_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."user_schedule_policies_projection"
    ADD CONSTRAINT "user_schedule_policies_projection_pkey" PRIMARY KEY ("id");
...
CREATE INDEX "idx_org_emails_email_id" ON "public"."organization_emails" USING "btree" ("email_id");
...
CREATE INDEX "idx_permission_implications_implies" ON "public"."permission_implications" USING "btree" ("implies_permission_id");



CREATE INDEX "idx_permission_implications_permission" ON "public"."permission_implications" USING "btree" ("permission_id");
...
CREATE INDEX "idx_user_client_assignments_client" ON "public"."user_client_assignments_projection" USING "btree" ("client_id", "assigned_until") WHERE ("is_active" = true);



CREATE INDEX "idx_user_client_assignments_org" ON "public"."user_client_assignments_projection" USING "btree" ("organization_id") WHERE ("is_active" = true);



CREATE INDEX "idx_user_client_assignments_user" ON "public"."user_client_assignments_projection" USING "btree" ("user_id", "assigned_until") WHERE ("is_active" = true);
...
CREATE INDEX "idx_user_schedule_policies_dates" ON "public"."user_schedule_policies_projection" USING "btree" ("effective_from", "effective_until") WHERE ("is_active" = true);



CREATE INDEX "idx_user_schedule_policies_org_ou" ON "public"."user_schedule_policies_projection" USING "btree" ("organization_id", "org_unit_id") WHERE ("is_active" = true);



CREATE INDEX "idx_user_schedule_policies_org_unit" ON "public"."user_schedule_policies_projection" USING "btree" ("org_unit_id");



CREATE INDEX "idx_user_schedule_policies_user" ON "public"."user_schedule_policies_projection" USING "btree" ("user_id") WHERE ("is_active" = true);



CREATE INDEX "idx_users_current_org_unit" ON "public"."users" USING "btree" ("current_org_unit_id") WHERE ("current_org_unit_id" IS NOT NULL);
...
ALTER TABLE ONLY "public"."cross_tenant_access_grants_projection"
    ADD CONSTRAINT "fk_cross_tenant_grants_consultant_org" FOREIGN KEY ("consultant_org_id") REFERENCES "public"."organizations_projection"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."cross_tenant_access_grants_projection"
    ADD CONSTRAINT "fk_cross_tenant_grants_provider_org" FOREIGN KEY ("provider_org_id") REFERENCES "public"."organizations_projection"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."impersonation_sessions_projection"
    ADD CONSTRAINT "fk_impersonation_sessions_target_org" FOREIGN KEY ("target_org_id") REFERENCES "public"."organizations_projection"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."user_notification_preferences_projection"
    ADD CONSTRAINT "fk_user_notification_prefs_org" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations_projection"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."permission_implications"
    ADD CONSTRAINT "permission_implications_implies_permission_id_fkey" FOREIGN KEY ("implies_permission_id") REFERENCES "public"."permissions_projection"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."permission_implications"
    ADD CONSTRAINT "permission_implications_permission_id_fkey" FOREIGN KEY ("permission_id") REFERENCES "public"."permissions_projection"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."user_client_assignments_projection"
    ADD CONSTRAINT "user_client_assignments_projection_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations_projection"("id");



ALTER TABLE ONLY "public"."user_client_assignments_projection"
    ADD CONSTRAINT "user_client_assignments_projection_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id");
...
ALTER TABLE ONLY "public"."user_schedule_policies_projection"
    ADD CONSTRAINT "user_schedule_policies_projection_org_unit_id_fkey" FOREIGN KEY ("org_unit_id") REFERENCES "public"."organization_units_projection"("id");



ALTER TABLE ONLY "public"."user_schedule_policies_projection"
    ADD CONSTRAINT "user_schedule_policies_projection_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations_projection"("id");



ALTER TABLE ONLY "public"."user_schedule_policies_projection"
    ADD CONSTRAINT "user_schedule_policies_projection_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id");



ALTER TABLE ONLY "public"."users"
    ADD CONSTRAINT "users_current_org_unit_id_fkey" FOREIGN KEY ("current_org_unit_id") REFERENCES "public"."organization_units_projection"("id");
...
CREATE POLICY "domain_events_authenticated_insert" ON "public"."domain_events" FOR INSERT WITH CHECK (((( SELECT "auth"."uid"() AS "uid") IS NOT NULL) AND ("public"."has_platform_privilege"() OR ((("event_metadata" ->> 'organization_id'::"text"))::"uuid" = (( SELECT (("current_setting"('request.jwt.claims'::"text", true))::"jsonb" ->> 'org_id'::"text")))::"uuid")) AND ("length"(("event_metadata" ->> 'reason'::"text")) >= 10)));



CREATE POLICY "domain_events_org_select" ON "public"."domain_events" FOR SELECT USING (((( SELECT "auth"."uid"() AS "uid") IS NOT NULL) AND ("public"."has_platform_privilege"() OR ((("event_metadata" ->> 'organization_id'::"text"))::"uuid" = (( SELECT (("current_setting"('request.jwt.claims'::"text", true))::"jsonb" ->> 'org_id'::"text")))::"uuid"))));
...
CREATE POLICY "impersonation_sessions_own_sessions_select" ON "public"."impersonation_sessions_projection" FOR SELECT USING ((("super_admin_user_id" = (( SELECT "current_setting"('app.current_user'::"text", true) AS "current_setting"))::"uuid") OR ("target_user_id" = (( SELECT "current_setting"('app.current_user'::"text", true) AS "current_setting"))::"uuid")));
...
CREATE POLICY "impersonation_sessions_provider_admin_select" ON "public"."impersonation_sessions_projection" FOR SELECT USING ((("target_org_id" = (( SELECT "current_setting"('app.current_org'::"text", true) AS "current_setting"))::"uuid") AND (EXISTS ( SELECT 1
   FROM ("public"."user_roles_projection" "ur"
     JOIN "public"."roles_projection" "r" ON (("r"."id" = "ur"."role_id")))
  WHERE (("ur"."user_id" = (( SELECT "current_setting"('app.current_user'::"text", true) AS "current_setting"))::"uuid") AND ("r"."name" = 'provider_admin'::"text") AND ("ur"."organization_id" = "impersonation_sessions_projection"."target_org_id"))))));
...
CREATE POLICY "impersonation_sessions_super_admin_select" ON "public"."impersonation_sessions_projection" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM ("public"."user_roles_projection" "ur"
     JOIN "public"."roles_projection" "r" ON (("r"."id" = "ur"."role_id")))
  WHERE (("ur"."user_id" = (( SELECT "current_setting"('app.current_user'::"text", true) AS "current_setting"))::"uuid") AND ("r"."name" = 'super_admin'::"text") AND ("ur"."organization_id" IS NULL)))));
...
CREATE POLICY "invitations_user_own_select" ON "public"."invitations_projection" FOR SELECT USING (("email" = ( SELECT (("current_setting"('request.jwt.claims'::"text", true))::json ->> 'email'::"text"))));
...
CREATE POLICY "organizations_scope_delete" ON "public"."organizations_projection" FOR DELETE USING (("public"."has_effective_permission"('organization.delete'::"text", "path") AND ("extensions"."nlevel"("path") > 2)));



COMMENT ON POLICY "organizations_scope_delete" ON "public"."organizations_projection" IS 'Allows users to delete sub-organizations within their permission scope.
Root org deletion requires platform admin.';



CREATE POLICY "organizations_scope_insert" ON "public"."organizations_projection" FOR INSERT WITH CHECK (("public"."has_effective_permission"('organization.create'::"text", "path") AND ("extensions"."nlevel"("path") > 2)));



COMMENT ON POLICY "organizations_scope_insert" ON "public"."organizations_projection" IS 'Allows users to create sub-organizations within their permission scope.
Root org creation (nlevel <= 2) requires platform admin via separate policy.';



CREATE POLICY "organizations_scope_select" ON "public"."organizations_projection" FOR SELECT USING ("public"."has_effective_permission"('organization.view'::"text", "path"));



COMMENT ON POLICY "organizations_scope_select" ON "public"."organizations_projection" IS 'Allows users to view organizations within their effective permission scope.
Uses has_effective_permission() which checks JWT effective_permissions array.';



CREATE POLICY "organizations_scope_update" ON "public"."organizations_projection" FOR UPDATE USING (("public"."has_effective_permission"('organization.update'::"text", "path") AND ("extensions"."nlevel"("path") > 2))) WITH CHECK (("public"."has_effective_permission"('organization.update'::"text", "path") AND ("extensions"."nlevel"("path") > 2)));



COMMENT ON POLICY "organizations_scope_update" ON "public"."organizations_projection" IS 'Allows users to update sub-organizations within their permission scope.
...
CREATE POLICY "ou_scope_delete" ON "public"."organization_units_projection" FOR DELETE USING ("public"."has_effective_permission"('organization.delete_ou'::"text", "path"));



COMMENT ON POLICY "ou_scope_delete" ON "public"."organization_units_projection" IS 'Allows users to delete organization units within their permission scope.';



CREATE POLICY "ou_scope_insert" ON "public"."organization_units_projection" FOR INSERT WITH CHECK ("public"."has_effective_permission"('organization.create_ou'::"text", "path"));



COMMENT ON POLICY "ou_scope_insert" ON "public"."organization_units_projection" IS 'Allows users to create organization units within their permission scope.';



CREATE POLICY "ou_scope_select" ON "public"."organization_units_projection" FOR SELECT USING ("public"."has_effective_permission"('organization.view_ou'::"text", "path"));



COMMENT ON POLICY "ou_scope_select" ON "public"."organization_units_projection" IS 'Allows users to view organization units within their effective permission scope.';



CREATE POLICY "ou_scope_update" ON "public"."organization_units_projection" FOR UPDATE USING ("public"."has_effective_permission"('organization.update_ou'::"text", "path")) WITH CHECK ("public"."has_effective_permission"('organization.update_ou'::"text", "path"));



COMMENT ON POLICY "ou_scope_update" ON "public"."organization_units_projection" IS 'Allows users to update organization units within their permission scope.';
...
CREATE POLICY "permission_implications_modify" ON "public"."permission_implications" USING ((( SELECT (("current_setting"('request.jwt.claims'::"text", true))::"jsonb" ->> 'user_role'::"text")) = 'super_admin'::"text"));



CREATE POLICY "permission_implications_select" ON "public"."permission_implications" FOR SELECT USING (true);
...
CREATE POLICY "role_permission_templates_write" ON "public"."role_permission_templates" USING ((EXISTS ( SELECT 1
   FROM ("public"."user_roles_projection" "ur"
     JOIN "public"."roles_projection" "r" ON (("r"."id" = "ur"."role_id")))
  WHERE (("ur"."user_id" = ( SELECT "auth"."uid"() AS "uid")) AND ("r"."name" = 'super_admin'::"text")))));
...
CREATE POLICY "user_client_assignments_modify" ON "public"."user_client_assignments_projection" USING ("public"."has_effective_permission"('user.client_assign'::"text", ( SELECT "organizations_projection"."path"
   FROM "public"."organizations_projection"
  WHERE ("organizations_projection"."id" = "user_client_assignments_projection"."organization_id"))));



ALTER TABLE "public"."user_client_assignments_projection" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "user_client_assignments_select" ON "public"."user_client_assignments_projection" FOR SELECT USING (("organization_id" = "public"."get_current_org_id"()));
...
CREATE POLICY "user_notification_prefs_select_own" ON "public"."user_notification_preferences_projection" FOR SELECT USING ((("user_id" = ( SELECT "auth"."uid"() AS "uid")) OR ((((( SELECT "auth"."jwt"() AS "jwt") -> 'app_metadata'::"text") ->> 'org_id'::"text"))::"uuid" = "organization_id")));



CREATE POLICY "user_notification_prefs_service_role" ON "public"."user_notification_preferences_projection" USING ((( SELECT "current_setting"('role'::"text", true) AS "current_setting") = 'service_role'::"text"));



CREATE POLICY "user_notification_prefs_update_own" ON "public"."user_notification_preferences_projection" FOR UPDATE USING (("user_id" = ( SELECT "auth"."uid"() AS "uid"))) WITH CHECK (("user_id" = ( SELECT "auth"."uid"() AS "uid")));
...
CREATE POLICY "user_schedule_policies_modify" ON "public"."user_schedule_policies_projection" USING ("public"."has_effective_permission"('user.schedule_manage'::"text", COALESCE(( SELECT "organization_units_projection"."path"
   FROM "public"."organization_units_projection"
  WHERE ("organization_units_projection"."id" = "user_schedule_policies_projection"."org_unit_id")), ( SELECT "organizations_projection"."path"
   FROM "public"."organizations_projection"
  WHERE ("organizations_projection"."id" = "user_schedule_policies_projection"."organization_id")))));



ALTER TABLE "public"."user_schedule_policies_projection" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "user_schedule_policies_select" ON "public"."user_schedule_policies_projection" FOR SELECT USING (("organization_id" = "public"."get_current_org_id"()));
...
CREATE POLICY "users_select" ON "public"."users" FOR SELECT USING (("public"."has_platform_privilege"() OR ("id" = ( SELECT "auth"."uid"() AS "uid")) OR ("current_organization_id" = "public"."get_current_org_id"())));
...
CREATE POLICY "workflow_queue_service_role_all" ON "public"."workflow_queue_projection" TO "service_role" USING (true) WITH CHECK (true);
...
GRANT ALL ON FUNCTION "api"."assign_client_to_user"("p_user_id" "uuid", "p_client_id" "uuid", "p_assigned_until" timestamp with time zone, "p_notes" "text", "p_reason" "text") TO "authenticated";
...
GRANT ALL ON FUNCTION "api"."create_user_schedule"("p_user_id" "uuid", "p_schedule_name" "text", "p_schedule" "jsonb", "p_org_unit_id" "uuid", "p_effective_from" "date", "p_effective_until" "date", "p_reason" "text") TO "authenticated";
...
GRANT ALL ON FUNCTION "api"."deactivate_user_schedule"("p_schedule_id" "uuid", "p_reason" "text") TO "authenticated";
...
GRANT ALL ON FUNCTION "api"."delete_user_schedule"("p_schedule_id" "uuid", "p_reason" "text") TO "authenticated";
...
GRANT ALL ON FUNCTION "api"."get_current_org_unit"() TO "authenticated";
...
GRANT ALL ON FUNCTION "api"."get_organization_direct_care_settings"("p_org_id" "uuid") TO "authenticated";
...
GRANT ALL ON FUNCTION "api"."get_schedule_by_id"("p_schedule_id" "uuid") TO "authenticated";
...
GRANT ALL ON FUNCTION "api"."list_user_client_assignments"("p_org_id" "uuid", "p_user_id" "uuid", "p_client_id" "uuid", "p_active_only" boolean) TO "authenticated";



GRANT ALL ON FUNCTION "api"."list_user_schedules"("p_org_id" "uuid", "p_user_id" "uuid", "p_org_unit_id" "uuid", "p_active_only" boolean, "p_schedule_name" "text") TO "authenticated";
...
GRANT ALL ON FUNCTION "api"."reactivate_user_schedule"("p_schedule_id" "uuid", "p_reason" "text") TO "authenticated";
...
GRANT ALL ON FUNCTION "api"."switch_org_unit"("p_org_unit_id" "uuid") TO "authenticated";



GRANT ALL ON FUNCTION "api"."unassign_client_from_user"("p_user_id" "uuid", "p_client_id" "uuid", "p_reason" "text") TO "authenticated";



GRANT ALL ON FUNCTION "api"."update_organization_direct_care_settings"("p_org_id" "uuid", "p_enable_staff_client_mapping" boolean, "p_enable_schedule_enforcement" boolean) TO "authenticated";



GRANT ALL ON FUNCTION "api"."update_organization_direct_care_settings"("p_org_id" "uuid", "p_enable_staff_client_mapping" boolean, "p_enable_schedule_enforcement" boolean, "p_reason" "text") TO "authenticated";
...
GRANT ALL ON FUNCTION "api"."update_user_schedule"("p_schedule_id" "uuid", "p_schedule_name" "text", "p_schedule" "jsonb", "p_org_unit_id" "uuid", "p_effective_from" "date", "p_effective_until" "date", "p_reason" "text") TO "authenticated";
...
GRANT ALL ON FUNCTION "public"."get_clients_assigned_to_user"("p_user_id" "uuid", "p_org_id" "uuid") TO "authenticated";
...
GRANT ALL ON FUNCTION "public"."get_permission_scope"("p_permission" "text") TO "authenticated";



GRANT ALL ON FUNCTION "public"."get_staff_assigned_to_client"("p_client_id" "uuid", "p_org_id" "uuid") TO "authenticated";
...
GRANT ALL ON FUNCTION "public"."is_user_assigned_to_client"("p_user_id" "uuid", "p_client_id" "uuid") TO "authenticated";



GRANT ALL ON FUNCTION "public"."is_user_on_schedule"("p_user_id" "uuid", "p_org_id" "uuid", "p_org_unit_id" "uuid", "p_check_time" timestamp with time zone) TO "authenticated";
