-- Migration: Align process_user_event() notification preferences handler with AsyncAPI contract
--
-- AsyncAPI contract (source of truth) uses:
--   - notification_preferences (not 'preferences')
--   - notification_preferences.sms.phone_id (snake_case per contract)
--   - notification_preferences.in_app (snake_case per contract)
--
-- This migration updates the handler to match the contract.
-- We also add backwards compatibility for existing camelCase events during transition.

-- ============================================================================
-- Update only the notification preferences handler section
-- ============================================================================
...
BEGIN
  CASE p_event.event_type
    ...
    --   - notification_preferences.sms.phone_id (snake_case per contract)
    --   - notification_preferences.in_app (snake_case per contract)
    -- BACKWARDS COMPATIBLE: Also accepts camelCase during transition
    -- =========================================================================
    ...
    WHEN 'user.notification_preferences.updated' THEN
      ...
      -- Extract sms.phone_id with backwards compatibility for camelCase
      -- Priority: snake_case (contract) > camelCase (legacy)
      v_sms_phone_id := COALESCE(
        NULLIF(p_event.event_data->'notification_preferences'->'sms'->>'phone_id', '')::UUID,
        NULLIF(p_event.event_data->'notification_preferences'->'sms'->>'phoneId', '')::UUID
      );
      ...
      -- Extract in_app with backwards compatibility for camelCase
...
AsyncAPI Alignment (2026-01-19):
- notification_preferences handler aligned with AsyncAPI contract
- Uses snake_case per contract: phone_id, in_app
- Backwards compatible: accepts camelCase (phoneId, inApp) during transition
...
- 2026-01-19: Aligned with AsyncAPI contract (notification_preferences, snake_case)';


-- ============================================================================
-- Reprocess the failed notification preferences event
-- ============================================================================
...
BEGIN
  -- Find and reprocess the notification_preferences event from user's test
  FOR v_failed_event IN
    SELECT * FROM domain_events
    WHERE event_type = 'user.notification_preferences.updated'
      AND (processing_error IS NOT NULL OR processed_at < '2026-01-19 18:20:00')
    ORDER BY created_at DESC
    LIMIT 5
  LOOP
    RAISE NOTICE 'Reprocessing event: % (id: %)', v_failed_event.event_type, v_failed_event.id;
