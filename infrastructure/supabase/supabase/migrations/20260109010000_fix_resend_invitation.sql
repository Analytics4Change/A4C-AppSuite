-- Fix: Resend invitation creates duplicates instead of updating
--
-- Root cause:
-- 1. api.get_invitation_for_resend() returns `id` (auto-gen) but edge function
--    uses it as `invitation_id` in the event
-- 2. Event processor does ON CONFLICT (invitation_id) DO NOTHING instead of UPDATE
--
-- This migration:
-- A) Adds invitation_id to RPC return type
-- B) Changes event processor to UPDATE on conflict (for resend support)

-- ============================================================================
-- Part A: Fix api.get_invitation_for_resend() to return invitation_id
-- ============================================================================

DROP FUNCTION IF EXISTS api.get_invitation_for_resend(UUID, UUID);
...
COMMENT ON FUNCTION api.get_invitation_for_resend(UUID, UUID) IS
  'Get invitation details for resend operation. Returns both id and invitation_id for proper event correlation.';
...
-- ============================================================================
-- Part B: Update process_organization_event to UPDATE on conflict for resend
-- ============================================================================
-- We need to replace the user.invited handler in process_organization_event
...
BEGIN
  CASE p_event.event_type
    ...
    -- ========================================
    -- user.invited (with UPDATE for resend support)
    -- ========================================
    WHEN 'user.invited' THEN
      ...
      ON CONFLICT (invitation_id) DO UPDATE SET
        -- Update fields that change on resend
        token = EXCLUDED.token,
        expires_at = EXCLUDED.expires_at,
        status = 'pending',  -- Reset status on resend (in case it was expired)
...
COMMENT ON FUNCTION public.process_organization_event(record) IS
  'Organization event processor with resend support for user.invited events';
