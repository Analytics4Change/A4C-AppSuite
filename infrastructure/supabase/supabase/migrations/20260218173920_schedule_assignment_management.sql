-- =============================================================================
-- Migration: Schedule Assignment Management
-- Purpose: Add 0-or-1 schedule constraint, list users for management, sync assignments
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. UNIQUE constraint: A user can be on at most ONE schedule per organization
-- -----------------------------------------------------------------------------
ALTER TABLE schedule_user_assignments_projection
  ADD CONSTRAINT schedule_user_assignments_unique_user_per_org
  UNIQUE (user_id, organization_id);

-- -----------------------------------------------------------------------------
-- 2. Function: api.list_users_for_schedule_management
-- Purpose: List ALL org users with assignment status for schedule management UI
-- Returns: Users with is_assigned flag and current schedule info (if on another)
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  SELECT t.id, t.name, t.org_unit_id INTO v_template
  ...
  ORDER BY
    is_assigned DESC,        -- Assigned to THIS template first
...
GRANT EXECUTE ON FUNCTION api.list_users_for_schedule_management(UUID, TEXT, INT, INT) TO authenticated;


-- -----------------------------------------------------------------------------
-- 3. Function: api.sync_schedule_assignments
-- Purpose: Add AND remove schedule assignments in a single operation
--          Handles auto-transfer: if a user is on another template, unassign first
-- Returns: Combined result with successes/failures for both operations + transfers
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Validate template exists, belongs to org, and is not deleted
  SELECT t.id, t.name, t.org_unit_id INTO v_template
  FROM schedule_templates_projection t
  WHERE t.id = p_template_id
    AND t.organization_id = v_org_id
  ...
  IF p_user_ids_to_add IS NOT NULL THEN
    FOREACH v_user_id IN ARRAY p_user_ids_to_add LOOP
      ...
      BEGIN
        ...
        SELECT sua.schedule_template_id, st.name AS template_name
        ...
        IF FOUND THEN
          ...
          v_event_metadata := jsonb_build_object(
            ...
            'reason', 'Auto-transfer to template: ' || v_template.name,
            'source', 'api',
...
GRANT EXECUTE ON FUNCTION api.sync_schedule_assignments(UUID, UUID[], UUID[], UUID, TEXT) TO authenticated;
